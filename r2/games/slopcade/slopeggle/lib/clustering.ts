...
import type { SlopegglePeg } from "./generator/types";
 ...
 */
function calculateClusterCenter(pegs: SlopegglePeg[]): {
	x: number;
	y: number;
} {
	if (pegs.length === 0) {
 ...
 */
export function positionToPhase(
	x: number,
	y: number,
	clusterPhase: number,
): number {
...
export function getClusteringStats(
	...
	return {
		...
		smallestClusterSize: clusters.length > 0 ? Math.min(...clusterSizes) : 0,
	};
}
