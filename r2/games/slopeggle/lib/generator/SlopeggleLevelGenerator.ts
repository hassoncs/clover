import {
	...
	type SeededRandom,
} from "@slopcade/shared/generator/SeededRandom";
import type {
...
import {
	isSlopeggleLevelOverlay,
	SLOPEGGLE_DIFFICULTY_PRESETS,
	SLOPEGGLE_WORLD_DEFAULTS,
} from "./types";
 ...
 */
export function generateSlopeggleLevel(
	...
	const minOrangeAccessibility =
		overrideMinAccessibility ?? preset.minOrangeAccessibility;
	...
	const overlay: SlopeggleLevelOverlay = {
		...
		description:
			description ?? `Clear ${orangeCount} orange pegs with ${lives} balls`,
		ordinal,
		generatedAt: Date.now(),
		difficulty: {
			targetTier: difficultyTier,
			lives: lives,
			estimatedDurationSeconds: estimateLevelDuration(
				orangeCount,
				lives,
				difficultyTier,
			),
...
function placeOrangePegs(
	...
	const guaranteedAccessible = Math.max(
		accessibleIndices.length > 0 ? minAccessible : 0,
		1,
	);
	...
	for (
		let i = 0;
		i < accessibleOrangeCount && i < shuffledAccessible.length;
		i++
	) {
	...
	const nonAccessibleIndices = getNonAccessiblePegIndices(
		pegs,
		accessibleIndices,
	);
	...
	for (
		let i = 0;
		i < nonAccessibleOrangeCount && i < shuffledNonAccessible.length;
		i++
	) {
...
function getNonAccessiblePegIndices(
	...
	return pegs.map((_, i) => i).filter((i) => !accessibleSet.has(i));
}
...
function estimateLevelDuration(
	...
	const estimatedSeconds =
		orangeCount * baseTimePerOrange * tierMultiplier * lifeFactor;
 ...
 */
export function verifyDeterminism(params: GenerateSlopeggleLevelParams): {
	deterministic: boolean;
	overlay1: SlopeggleLevelOverlay;
	overlay2: SlopeggleLevelOverlay;
} {
