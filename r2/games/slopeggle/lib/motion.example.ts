...
import type {
	Behavior,
	OscillateBehavior,
} from "@slopcade/shared/types/behavior";
import { clusterPegs, getClusteringStats, positionToPhase } from "./clustering";
import { generateSlopeggleLevel } from "./generator/SlopeggleLevelGenerator";
import { applyPegMotion, PegMotionAssigner } from "./PegMotionAssigner";
...
export function exampleMotionToBehaviors(): void {
	...
	const scaleCount = behaviors.filter(
		(b) => b.type === "scale_oscillate",
	).length;
...
export function exampleCustomConfig(): void {
	...
	console.log(
		`  Motion density: ${((result.stats.pegsWithMotion / result.stats.totalPegs) * 100).toFixed(1)}%`,
	);
...
export function exampleFairnessVerification(): void {
	...
	console.log(
		`    Max amplitude: ${maxAmplitude.toFixed(5)} (max allowed: ${maxAllowedAmplitude.toFixed(5)})`,
	);
	console.log(`    Max frequency: ${maxFrequency.toFixed(3)} (target: ~0.15)`);
	console.log(
		`    Amplitude within 10-15% of radius: ${((maxAmplitude / pegRadius) * 100).toFixed(1)}%`,
	);
	...
	console.log(
		"  (Motion does not affect collision detection - all pegs remain accessible)",
	);
}
...
export function exampleFullWorkflow(): void {
	...
	console.log(
		`  Motion applied to ${result.stats.pegsWithMotion} pegs (${(result.stats.pegsWithMotion / result.stats.totalPegs) * 100}%)`,
	);
	console.log(`  Clusters formed: ${result.stats.clustersFormed}\n`);
