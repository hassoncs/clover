...
import type { EffectGraphSpec, EffectNode, ParamValue, EffectParamSchema } from '@slopcade/shared';
...
export function EffectTuningPanel({ spec, onParamChange }: EffectTuningPanelProps) {
  const nodesWithParams = useMemo(() => {
    return spec.nodes.filter(node => node.paramsSchema && node.paramsSchema.length > 0);
  }, [spec.nodes]);

  if (nodesWithParams.length === 0) {
    return (
      <View className="p-4 items-center justify-center flex-1">
        <Text className="text-gray-500 text-center">No tunable parameters found in this effect graph.</Text>
      </View>
    );
  }
