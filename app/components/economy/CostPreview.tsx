  ...
  if (isLoading) {
    return (
      <View className="bg-theme-surface-elevated p-6 rounded-lg items-center justify-center">
        ...
        <Text className="text-theme-text-secondary mt-2 text-sm">Estimating cost...</Text>
      </View>
    );
  }
  ...
  if (error) {
    return (
      <View className="bg-theme-error/10 p-4 rounded-lg border border-theme-error/20">
        <Text className="text-theme-error text-center">Failed to load cost estimate</Text>
      </View>
    );
  }
  ...
  return (
    <View className="bg-theme-surface-elevated p-4 rounded-lg shadow-sm">
      ...
      <View className="mb-3">
        {data.breakdown.map((item) => (
          <View key={item.description} className="flex-row justify-between mb-1">
            ...
            <Text className={`text-sm font-medium ${item.totalMicros < 0 ? 'text-theme-success' : 'text-theme-text'}`}>
      ...
      <View className="flex-row justify-between items-center">
        ...
        <Text className={`font-bold ${data.canAfford ? 'text-theme-success' : 'text-theme-error'}`}>
