...
export function CurrencySheet({ visible, onClose }: CurrencySheetProps) {
	...
	return (
		<>
					...
					<ScrollView className="flex-1">
						<View className="p-4">
							<View className="bg-gradient-to-br from-purple-900 to-gray-800 rounded-2xl p-6 mb-4 border border-purple-700">
								...
								{activeBrand.features.aiGeneration && (
									<View className="border-t border-gray-700 pt-4">
										<View className="flex-row items-baseline">
											...
											<Text className="text-white text-3xl font-bold ml-2">
												{isLoading
													? "..."
													: (balance?.balanceSparks.toLocaleString() ?? "0")}
											</Text>
										...
										<Text className="text-gray-500 text-xs mt-1 ml-12">
											...
											{isLoading
												? "..."
												: balance?.balanceSparks
													? (balance.balanceSparks / 1000).toFixed(2)
													: "0.00"}{" "}
											compute credit
										</Text>
									</View>
									...
									<View>
										<View className="flex-row items-start">
											...
											<View className="flex-1">
												...
												<Text className="text-gray-400 text-sm">
													Compute credits for asset generation (sprites,
													backgrounds, audio)
												</Text>
											</View>
										</View>
			...
			{showSparksModal && activeBrand.features.aiGeneration && (
				<Suspense
					fallback={
						<Modal visible={true} transparent animationType="fade">
							<View className="flex-1 items-center justify-center bg-black/50">
