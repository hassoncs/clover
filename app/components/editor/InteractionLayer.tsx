...
export function InteractionLayer({
	...
	const screenToWorld = useCallback(
		(screenX: number, screenY: number): Vec2 => {
			...
			const worldX =
				cameraX.value + (screenX - centerX) / (pixelsPerMeter * zoom);
			const worldY =
				cameraY.value + (screenY - centerY) / (pixelsPerMeter * zoom);
	...
	const findEntityAtPoint = useCallback(
		(worldPos: Vec2): string | null => {
			...
			for (let i = document.entities.length - 1; i >= 0; i--) {
				...
				const prefab = document.prefabs[entity.prefab];
				if (!prefab) continue;
	...
	const handlePinchUpdate = useCallback(
		(pinchScale: number) => {
			...
			const newScale = Math.max(
				0.25,
				Math.min(4, initialScaleRef.current * pinchScale),
			);
	...
	const cameraPanGesture = Gesture.Pan()
		.minPointers(2)
		.onUpdate((event) => {
			runOnJS(handleCameraPan)(
				event.translationX * 0.1,
				event.translationY * 0.1,
			);
		});
