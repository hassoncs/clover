...
export default function TestGameRunScreen() {
  ...
  const resolvedPackEntries = useMemo(() => {
    ...
    console.log(
      "[test-games] Resolving asset pack entries:",
    ...
    console.log(
      "[test-games] Total resolved assets:",
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      try {
        console.log(
          "[test-games] Loading game from embedded registry:",
          id,
          "level:",
          currentLevel
        );
  ...
  const handlePreviousLevel = useCallback(() => {
    console.log(
      "[test-games] Previous level requested, current:",
      currentLevel
    );
    if (currentLevel > 1) {
  ...
  return (
    <View className="flex-1 bg-gray-900">
      ...
      {canMountGame &&
        (() => {
          console.log(
            "[test-games] 🚀 Mounting GameRuntime (assets merged into definition)"
          );
          return (
            <GameRuntimeWrapper
              key={runtimeKey}
