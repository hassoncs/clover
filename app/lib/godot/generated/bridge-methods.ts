...
export function createBridgeMethods(dispatch: PlatformDispatch): Partial<GodotBridge> {
  ...
  async loadGame(definition: GameDefinition): Promise<void> {
    return dispatch.async<void>("load_game_json", [JSON.stringify(definition)]);
  },
  ...
  setupWorld(world: GameDefinition["world"], background?: GameDefinition["background"]): void {
    dispatch.sync("setup_world", world?.gravity?.x ?? 0, world?.gravity?.y ?? 0, world?.pixelsPerMeter ?? 0, world?.bounds?.width ?? 0, world?.bounds?.height ?? 0, JSON.stringify(background));
  },
  ...
  spawnEntity(request: SpawnEntityRequest): void {
    dispatch.sync("spawn_entity", request?.entityId ?? "", request?.prefabId ?? "", request?.position?.x ?? 0, request?.position?.y ?? 0, request?.velocity?.x ?? 0, request?.velocity?.y ?? 0);
  },
  ...
  createRevoluteJoint(def: RevoluteJointDef): number {
    return dispatch.sync("create_revolute_joint", def?.type ?? "", def?.bodyA ?? "", def?.bodyB ?? "", def?.anchor?.x ?? 0, def?.anchor?.y ?? 0, def?.enableLimit ?? false, def?.lowerAngle ?? 0, def?.upperAngle ?? 0, def?.enableMotor ?? false, def?.motorSpeed ?? 0, def?.maxMotorTorque ?? 0) as number ?? -1;
  },

  createDistanceJoint(def: DistanceJointDef): number {
    return dispatch.sync("create_distance_joint", def?.type ?? "", def?.bodyA ?? "", def?.bodyB ?? "", def?.anchorA?.x ?? 0, def?.anchorA?.y ?? 0, def?.anchorB?.x ?? 0, def?.anchorB?.y ?? 0, def?.length ?? 0, def?.stiffness ?? 0, def?.damping ?? 0) as number ?? -1;
  },

  createPrismaticJoint(def: PrismaticJointDef): number {
    return dispatch.sync("create_prismatic_joint", def?.type ?? "", def?.bodyA ?? "", def?.bodyB ?? "", def?.anchor?.x ?? 0, def?.anchor?.y ?? 0, def?.axis?.x ?? 0, def?.axis?.y ?? 0, def?.enableLimit ?? false, def?.lowerTranslation ?? 0, def?.upperTranslation ?? 0, def?.enableMotor ?? false, def?.motorSpeed ?? 0, def?.maxMotorForce ?? 0) as number ?? -1;
  },

  createWeldJoint(def: WeldJointDef): number {
    return dispatch.sync("create_weld_joint", def?.type ?? "", def?.bodyA ?? "", def?.bodyB ?? "", def?.anchor?.x ?? 0, def?.anchor?.y ?? 0, def?.stiffness ?? 0, def?.damping ?? 0) as number ?? -1;
  },

  createMouseJoint(def: MouseJointDef): number {
    return dispatch.sync("create_mouse_joint", def?.type ?? "", def?.body ?? "", def?.target?.x ?? 0, def?.target?.y ?? 0, def?.maxForce ?? 0, def?.stiffness ?? 0, def?.damping ?? 0) as number ?? -1;
  },

  async createMouseJointAsync(def: MouseJointDef): Promise<number> {
    return await dispatch.async<number>("create_mouse_joint_async", [def?.type ?? "", def?.body ?? "", def?.target?.x ?? 0, def?.target?.y ?? 0, def?.maxForce ?? 0, def?.stiffness ?? 0, def?.damping ?? 0]) ?? 0;
  },
  ...
  setDebugSettings(settings: {
    ...
    dispatch.sync("set_debug_settings", settings?.showInputDebug ?? false, settings?.showPhysicsShapes ?? false, settings?.showZones ?? false, settings?.showFPS ?? false);
  },
