...
export interface BridgeCallbackArrays {
  ...
  inputEvent: ((type: string, x: number, y: number, entityId: string | null) => void)[];
  uiButton: ((eventType: 'button_down' | 'button_up' | 'button_pressed', buttonId: string) => void)[];
...
export function createCallbackMethods(cbs: BridgeCallbackArrays) {
  return {
    ...
    onTransformSync(callback: (transforms: Record<string, EntityTransform>) => void) {
    ...
    onInputEvent(callback: (type: string, x: number, y: number, entityId: string | null) => void) {
      return subscribe(cbs.inputEvent, callback);
    },
    onUIButtonEvent(callback: (eventType: 'button_down' | 'button_up' | 'button_pressed', buttonId: string) => void) {
