	...
	const startRecording = useCallback(async () => {
		...
		try {
			...
			ws.onopen = () => {
				...
				startAudioCapture(onData, onVolume);
			...
			ws.onmessage = (event) => {
				...
				const msg = raw as RealtimeEvent;
				...
				if (msg.type === "conversation.item.input_audio_transcription.delta") {
					...
					console.error("[Speech] Transcription failed:", raw.error);
					setError({
						code: "OPENAI_ERROR",
						message: msg.error?.message ?? "Transcription failed",
					});
				} else if (msg.type === "error") {
					console.error("[Speech] OpenAI error:", raw.error);
