...
function createMockScriptContext(
...
describe("UnsafeScriptSandbox", () => {
	...
	describe("hot reload", () => {
		it("should reload script with new code", async () => {
			sandbox = new UnsafeScriptSandbox({
				scriptCode:
					'exports.onStart = function(ctx) { ctx.setVariable("v", 1); };',
				scriptId: "test-script",
				gameId: "test-game",
			});
	...
	describe("entity operations", () => {
        ...
        `,
			...
			entityManager.registerPrefab({ id: "ball" });
	...
	describe("console logging", () => {
        ...
        `,
			...
			expect(consoleLogSpy).toHaveBeenCalledWith("[Script]", "Value:", 42, {
				key: "test",
			});
		});
	...
	describe("async boundary invariants", () => {
        ...
        `,
			...
			expect(runtime.setVariable).toHaveBeenCalledWith(
				"waitReturnsPromise",
				true,
			);
		});
        ...
        `,
			...
			expect(result.success).toBe(true);
			expect(runtime.worldAsync.animate).toHaveBeenCalledWith(
				"entity_1",
				{ x: 10, y: 20 },
				{ duration: 200 },
			);
			expect(runtime.setVariable).toHaveBeenCalledWith(
				"animateReturnsPromise",
				true,
			);
		});
        ...
        `,
			...
			expect(result.success).toBe(true);
			expect(runtime.startSequence).toHaveBeenCalledWith(
				"intro-sequence",
				expect.any(Function),
			);
