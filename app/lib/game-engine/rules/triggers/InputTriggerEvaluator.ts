		...
		>
{
	evaluate(
		...
		switch (trigger.type) {
			case "tap": {
				...
				if (trigger.target && trigger.target !== "screen") {
					...
					if (targetEntityId) {
						...
						const hasTag = context.entityManager.hasTag(
							targetEntityId,
							trigger.target,
						);
						if (hasTag) {
				...
				if (
					trigger.xMinPercent !== undefined ||
					trigger.xMaxPercent !== undefined
				) {
					const viewportBounds = context.camera?.getViewportWorldBounds();
					if (viewportBounds) {
						const viewportWidth = viewportBounds.maxX - viewportBounds.minX;
						const tapPercent =
							((tapWorldX - viewportBounds.minX) / viewportWidth) * 100;

						if (
							trigger.xMinPercent !== undefined &&
							tapPercent < trigger.xMinPercent
						) {
							return false;
						}
						if (
							trigger.xMaxPercent !== undefined &&
							tapPercent >= trigger.xMaxPercent
						) {
							return false;
						}
					}
			...
			case "tilt": {
				...
				if (trigger.axis === "x")
					return this.checkTiltValue(context.input.tilt.x, threshold, dir);
				if (trigger.axis === "y")
					return this.checkTiltValue(context.input.tilt.y, threshold, dir);
				return (
					this.checkTiltValue(context.input.tilt.x, threshold, dir) ||
					this.checkTiltValue(context.input.tilt.y, threshold, dir)
				);
			}
	...
	private checkTiltValue(
		value: number,
		threshold: number,
		direction: "positive" | "negative" | "any" | undefined,
	): boolean {
	...
	private isPointInEntity(
		...
		if (collider.shape === "box" && collider.width && collider.height) {
			...
			return (
				x >= ex - halfW && x <= ex + halfW && y >= ey - halfH && y <= ey + halfH
			);
		}
