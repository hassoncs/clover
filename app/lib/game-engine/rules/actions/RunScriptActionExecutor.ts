...
export class RunScriptActionExecutor
	implements ActionExecutor<RunScriptAction>
{
	...
	execute(action: RunScriptAction, context: RuleContext): void {
		if (!this.sandbox) {
			console.warn(
				"[RunScriptActionExecutor] No script sandbox available - sandbox not set!",
			);
			return;
		}
		...
		if (!result.success && result.error) {
			console.error(
				`[RunScriptActionExecutor] Script error in '${functionName}':`,
	...
	private createScriptContext(
		...
		const getConstant = (
			...
			if (
				...
				name in context.evalContext.variables
			) {
		...
		const spawnEntity = (
			...
			entityManager.cacheEntity(entityId, prefabId, {
				x: position.x,
				y: position.y,
				angle: opts?.angle ?? 0,
				scaleX: 1,
		...
		const worldAsync: AsyncWorldOps = wOps
			...
			: {
					animate: async () => {
						console.warn(
							"[RunScriptActionExecutor] animate called but no worldOps!",
						);
					},
					wait: async () => {
						console.warn(
							"[RunScriptActionExecutor] wait called but no worldOps!",
						);
					},
				};
		...
		const inputSnapshot: InputSnapshot | null = context.inputEvents?.tap
			? {
					type: "tap",
					position: {
						x: context.inputEvents.tap.x,
						y: context.inputEvents.tap.y,
					},
		...
		const dragSnapshot: DragSnapshot | null = context.input?.drag
			? {
					isDragging: true,
					startPosition: {
						x: context.input.drag.startWorldX,
						y: context.input.drag.startWorldY,
					},
					currentPosition: {
						x: context.input.drag.currentWorldX,
						y: context.input.drag.currentWorldY,
					},
		...
		return {
			...
			startSequence: (
				...
				if (!wOps) {
					console.warn(
						"[RunScriptActionExecutor] startSequence unavailable: no worldOps on RuleContext",
					);
					return {
						name,
						get isRunning() {
			...
			mouse: context.input?.mouse
				? { x: context.input.mouse.x, y: context.input.mouse.y }
				: null,
			drag: dragSnapshot,
			random: () => seededRandom(),
