...
import type { WavesStartAction, WavesNextAction } from '@slopcade/shared';
import type { RuleContext } from '../types';

type WaveAction = WavesStartAction | WavesNextAction;

export class WaveActionExecutor implements ActionExecutor<WaveAction> {
  execute(action: WaveAction, context: RuleContext): void {
    switch (action.type) {
      case 'waves_start': this.executeStart(action, context); break;
      case 'waves_next': this.executeNext(action, context); break;
    }
  }
  ...
  private getWaveState(waveDefId: string, context: RuleContext): { currentWave: number; isActive: boolean; spawnedEntityIds: string[]; waveStartTime: number } {
    ...
    let states = context.mutator.getVariable(stateKey) as unknown as Record<string, { currentWave: number; isActive: boolean; spawnedEntityIds: string[]; waveStartTime: number }> | undefined;
    if (!states) states = {};
  ...
  private saveWaveState(waveDefId: string, state: { currentWave: number; isActive: boolean; spawnedEntityIds: string[]; waveStartTime: number }, context: RuleContext): void {
    const stateKey = '__waveStates';
    let states = context.mutator.getVariable(stateKey) as unknown as Record<string, typeof state> | undefined;
    if (!states) states = {};
    states[waveDefId] = state;
    context.mutator.setVariable(stateKey, states as unknown as number);
  }

  private executeStart(action: WavesStartAction, context: RuleContext): void {
  ...
  private executeNext(action: WavesNextAction, context: RuleContext): void {
    const state = this.getWaveState(action.waveDefId, context);
