...
import { getAllSystemExpressionFunctions, SystemPhase } from "@slopcade/shared";
...
export class BehaviorExecutorRuntimeSystem
	implements
		RuntimeSystem<BehaviorExecutorSystemConfig, BehaviorExecutorSystemState>
{
	...
	update(ctx: UpdateContext, _state: BehaviorExecutorSystemState): void {
		...
		if (!this.behaviorExecutor || !this.systemContext) {
			if (this.config.debug) {
				console.log(
					"[BehaviorExecutor] Early return - behaviorExecutor:",
		...
		const entities =
			this.systemContext.entityManager.getAllEntities() as RuntimeEntity[];
		...
		if (ballEntity && this.config.debug) {
			console.log(
				"[BehaviorExecutor] Total entities:",
				entities.length,
				"Active entities:",
				activeEntities.length,
			);
			console.log(
				"[BehaviorExecutor] Ball is active:",
				ballEntity.active,
				"Included in activeEntities:",
				activeEntities.includes(ballEntity),
			);
		}
	...
	private createBehaviorContext(
		ctx: UpdateContext,
	): Omit<BehaviorContext, "entity" | "resolveNumber" | "resolveVec2"> {
		if (!this.systemContext) {
			throw new Error(
				"[BehaviorExecutorRuntimeSystem] Cannot create context without systemContext",
			);
		}
		...
		return {
			...
			applySpriteEffect: async (_entityId: string, plan: any) => {
				await this.systemContext!.bridge.applyGraph(plan);
			},
			clearSpriteEffect: async (_entityId: string) => {
				await this.systemContext!.bridge.clearGraph();
			},
			updateSpriteEffectParams: async (
				_entityId: string,
				passId: string,
