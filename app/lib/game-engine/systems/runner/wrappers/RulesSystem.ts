...
import type {
	GameState as BehaviorGameState,
	InputEvents,
...
export class RulesSystem
	implements
		RuntimeSystem<RulesSystemConfig, RulesSystemState>,
		IGameStateMutator
{
	readonly id = "rules";
	...
	initialize(ctx: SystemContext, _config: RulesSystemConfig): void {
		...
		this.containerConditionEvaluator = new ContainerConditionEvaluator(
			containerSystem,
		);

		const containerActionExecutor = new ContainerActionExecutor(
			containerSystem,
		);
	...
	update(ctx: UpdateContext, _state: RulesSystemState): void {
		...
		try {
			...
			const smMeta =
				Object.keys(this.runtimeState.stateMachines).length > 0
			...
			if (inputEvents.gameLoaded || inputEvents.gameStarted) {
				logger.debug("rules", "RulesSystem received lifecycle events:", {
					gameLoaded: inputEvents.gameLoaded,
					gameStarted: inputEvents.gameStarted,
				});
			}
			...
			const ruleContext: RuleContext = {
				...
				playSound: (soundId: string, volume?: number, pitch?: number) =>
					this.systemContext!.bridge.playSound(soundId, volume, pitch),
				haptic: (style?: string) =>
					Haptics.impactAsync(
						(style as Haptics.ImpactFeedbackStyle) ?? "Medium",
					),
				hapticNotification: (style?: string) =>
					Haptics.notificationAsync(
						(style as "Success" | "Warning" | "Error") ?? "Success",
					),
				...
				setEntityTargetPosition: (
					...
					const distance = Math.sqrt(
						(x - entity.transform.x) ** 2 + (y - entity.transform.y) ** 2,
					);
					const duration =
						config?.duration ?? Math.min(0.3, Math.max(0.1, distance / 10));
			...
			for (const rule of this.rules) {
				...
				if (triggerResult) {
					...
					if (conditionsResult) {
						logger.debug(
							"rules",
							`Rule "${rule.id}" FIRED, executing ${rule.actions.length} actions:`,
	...
	setStateMachineDefinitions(
		stateMachines: StateMachineDefinition[] | undefined,
	): void {
	...
	private convertFrameInputEvents(
		frameEvents: readonly import("../types").InputEvent[],
	): InputEvents {
		if (frameEvents.length > 0) {
			logger.debug(
				"rules",
				`convertFrameInputEvents called with ${frameEvents.length} events:`,
				frameEvents.map((e) => e.type),
			);
		}
			...
			switch (event.type) {
				...
				case "drag_end":
					result.dragEnd = {
						...
						worldVelocityX: event.worldVelocityX,
						worldVelocityY: event.worldVelocityY,
					};
					break;
				case "button_pressed":
				...
				case "game_loaded":
					logger.debug(
						"rules",
						"convertFrameInputEvents: Found game_loaded event",
					);
		...
		for (const [eventName] of gameState.pendingEvents) {
			for (const [machineId, def] of Object.entries(this.smDefs)) {
				...
				for (const transition of def.transitions) {
					if (!this.transitionMatches(transition, currentState, eventName))
						continue;
	...
	private checkWinCondition(context: RuleContext): boolean {
		...
		try {
			...
			logger.warn(
				"rules",
				`WinCondition failed to evaluate: ${this.winCondition.expr}`,
				e,
			);
			return false;
		}
		...
		switch (this.loseCondition.type) {
			...
			case "custom": {
				...
				try {
					...
					logger.warn(
						"rules",
						`LoseCondition failed to evaluate: ${this.loseCondition.expr}`,
						e,
					);
					return false;
				}
