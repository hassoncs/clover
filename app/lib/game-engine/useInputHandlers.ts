...
export function useInputHandlers(
	...
	const sharedHandleKeyDown = useCallback(
		(e: KeyboardEvent) => {
			if (
	...
	const sharedHandleKeyUp = useCallback(
		(e: KeyboardEvent) => {
			if (
				lastKeyEventRef.current?.key === e.key &&
				lastKeyEventRef.current?.code === e.code &&
	...
	const handleTouchStart = useCallback(
		(event: GestureResponderEvent) => {
			...
			bridge.queryPointEntity(world).then((entityId) => {
				if (entityId && dragStartRef.current) {
					dragStartRef.current.targetEntityId = entityId;
				}
			});
		},
	...
	const handleTouchEnd = useCallback(
		(event: GestureResponderEvent) => {
			...
			// Push tap event to the JS event queue (mirrors handleClick behavior).
			// targetEntityId comes from dragStart which captured it at touch start.
			eventQueueRef.current.push({
				type: "tap",
				x,
				y,
	...
	const handleJoystickMove = useCallback(
		(state: JoystickState) => {
			...
			const bridge = bridgeRef.current;
			if (bridge?.setVirtualJoystick3D) {
				bridge.setVirtualJoystick3D(state.x, state.y);
			}
		},
		[inputRef, bridgeRef],
	);
	...
	const handleJoystickRelease = useCallback(() => {
		...
		const bridge = bridgeRef.current;
		if (bridge?.setVirtualJoystick3D) {
			bridge.setVirtualJoystick3D(0, 0);
		}
	}, [inputRef, bridgeRef]);
	...
	const handleTouchLook = useCallback(
		(deltaX: number, deltaY: number) => {
			const bridge = bridgeRef.current;
			if (bridge?.setTouchLook3D) {
				bridge.setTouchLook3D(deltaX, deltaY);
			}
		},
		[bridgeRef],
	);
