...
export function registerLifecycleBehaviors(executor: BehaviorExecutor): void {
	executor.registerHandler("spawn_on_event", (behavior, ctx, runtime) => {
		...
		switch (spawn.event) {
			...
			case "timer": {
				const interval =
					spawn.interval !== undefined ? ctx.resolveNumber(spawn.interval) : 1;
			...
			case "collision":
				for (const collision of ctx.collisions) {
					if (
						collision.entityA.id !== ctx.entity.id &&
						collision.entityB.id !== ctx.entity.id
					) {
						continue;
					}
					const other =
					...
					if (
						!spawn.withTags ||
						spawn.withTags.some((tag) => other.tags.includes(tag))
					) {
						shouldSpawn = true;
						break;
					}
			...
			switch (spawn.spawnPosition) {
				...
				case "random_in_bounds":
					if (spawn.bounds) {
						x =
							spawn.bounds.minX +
							Math.random() * (spawn.bounds.maxX - spawn.bounds.minX);
						y =
							spawn.bounds.minY +
							Math.random() * (spawn.bounds.maxY - spawn.bounds.minY);
					}
					break;
			}

			const prefabId = Array.isArray(spawn.entityTemplate)
				? spawn.entityTemplate[
						Math.floor(Math.random() * spawn.entityTemplate.length)
					]
				: spawn.entityTemplate;
	...
	executor.registerHandler("destroy_on_collision", (behavior, ctx, runtime) => {
		...
		for (const collision of ctx.collisions) {
			...
			const matchesTags = destroy.withTags.some((tag) =>
			...
			for (const childConfig of config.configs) {
				...
				if (childConfig.randomRange) {
					...
					const sourceChild = children.find(
						(c) => c.name === childConfig.offsetFrom,
					);
					if (!sourceChild) continue;

					const sourceValue =
						childConfig.property === "localTransform.y"
	...
	executor.registerHandler("score_on_collision", (behavior, ctx, runtime) => {
		...
		const scored =
			(runtime.state.scoredEntities as Set<string>) ?? new Set<string>();
		...
		for (const collision of ctx.collisions) {
			...
			const matchesTags = score.withTags.some((tag) =>
				other.tags.includes(tag),
			);
			...
			switch (timer.action) {
				...
				case "spawn":
					if (timer.spawnTemplate) {
						ctx.spawnEntity(
							timer.spawnTemplate,
							ctx.entity.transform.x,
							ctx.entity.transform.y,
						);
					}
					break;
				case "trigger_event":
				...
				case "disable_behavior":
					if (
						timer.behaviorIndex !== undefined &&
						ctx.entity.behaviors[timer.behaviorIndex]
					) {
						ctx.entity.behaviors[timer.behaviorIndex].enabled =
							timer.action === "enable_behavior";
					}
					break;
			}
	...
	executor.registerHandler("magnetic", (behavior, ctx) => {
		...
		for (const target of entities) {
			...
			if (
				magnetic.attractsTags &&
				!magnetic.attractsTags.some((tag) => target.tags.includes(tag))
			)
				continue;
			...
			const forceMagnitude =
				(strength * direction) / Math.max(dist * dist, 0.1);
	...
	executor.registerHandler("health", (behavior, ctx, runtime) => {
		...
		if (runtime.state.currentHealth === undefined) {
			runtime.state.currentHealth =
				health.currentHealth !== undefined
					? ctx.resolveNumber(health.currentHealth)
					: maxHealth;
		}

		const lastHitTime = (runtime.state.lastHitTime as number) ?? 0;
		const invulnerabilityTime =
			health.invulnerabilityTime !== undefined
		...
		if (!invulnerable && health.damageFromTags) {
			for (const collision of ctx.collisions) {
				...
				const matchesTags = health.damageFromTags.some((tag) =>
					other.tags.includes(tag),
				);
				if (!matchesTags) continue;
				...
				runtime.state.currentHealth =
					(runtime.state.currentHealth as number) - damage;
	...
	executor.registerHandler("particle_emitter", (behavior, ctx, runtime) => {
		...
		const offset = emitter.offset
			? ctx.resolveVec2(emitter.offset)
			: { x: 0, y: 0 };
		...
		if (!runtime.state.emitterId) {
			const shouldEmit =
				emitter.emitWhile === "always" ||
				emitter.emitWhile === undefined ||
				emitter.emitWhile === "enabled";
			if (shouldEmit) {
				runtime.state.emitterId = ctx.createEntityEmitter(
					emitter.emitterType,
					x,
					y,
				);
			...
			if (emitter.emitWhile === "moving") {
				...
				const isMoving =
					Math.abs(x - lastX) > 0.01 || Math.abs(y - lastY) > 0.01;
	...
	executor.registerHandler("attach_to", (behavior, ctx, runtime) => {
		...
		if (parentPrefab?.slots && attach.slotName in parentPrefab.slots) {
			const slot = parentPrefab.slots[attach.slotName];
			offsetX = slot.x;
			offsetY = slot.y;
	...
	executor.registerHandler("teleport", (behavior, ctx, runtime) => {
		...
		const cooldowns =
			(runtime.state.cooldowns as Map<string, number>) ??
		...
		for (const collision of ctx.collisions) {
			...
			const matchesTags = teleport.withTags.some((tag) =>
				other.tags.includes(tag),
			);
			if (!matchesTags) continue;
			...
			const destination = ctx.entityManager.getEntity(
				teleport.destinationEntityId,
			);
			if (!destination) {
				console.warn(
					`[Teleport] Destination entity not found: ${teleport.destinationEntityId}`,
				);
				continue;
			}
