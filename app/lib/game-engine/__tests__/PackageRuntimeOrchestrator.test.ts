...
const testArtifacts: Record<string, unknown> = {
  ...
  assets: { urls: { bg: 'https://cdn.example.com/bg.png' } } satisfies TagPayloads['assets'],
};
...
describe('PackageRuntimeOrchestrator', () => {
  ...
  describe('loadPackage', () => {
    ...
    it('provides default world when world artifact is missing', async () => {
      ...
      const noWorldOrch = new PackageRuntimeOrchestrator(bridge, noWorldResolver);
    ...
    it('reports bridge error when setupWorld fails', async () => {
      vi.mocked(bridge.setupWorld).mockImplementation(() => { throw new Error('Bridge crashed'); });
      const manifest = createManifest();
  ...
  describe('loadByTag', () => {
    ...
    it('calls clearEntities + loadEntities for entities tag', async () => {
      ...
      expect(bridge.loadEntities).toHaveBeenCalledTimes(1);
      const clearOrder = vi.mocked(bridge.clearEntities).mock.invocationCallOrder[0];
      const loadOrder = vi.mocked(bridge.loadEntities).mock.invocationCallOrder[0];
      expect(clearOrder).toBeLessThan(loadOrder);
    });

    it('does not call bridge for rules tag (local state only)', async () => {
      const manifest = createManifest();
      await orchestrator.loadPackage(manifest);
      vi.mocked(bridge.setupWorld).mockClear();
      vi.mocked(bridge.registerPrefabs).mockClear();
...
describe('Adapter ↔ Orchestrator parity', () => {
  function createPair() {
    ...
    const orchestrator = new PackageRuntimeOrchestrator(orchestratorBridge, orchestratorResolver);
  ...
  it('loadByTag: both fail identically when no manifest loaded', async () => {
    ...
    expect(adapterResult.errors![0].message).toBe(orchResult.errors![0].message);
  });
  ...
  it('adapter with tagNativeLoading flag delegates to orchestrator', async () => {
    ...
    const adapter = new PackageRuntimeAdapter(bridge, resolver, { tagNativeLoading: true });
  ...
  it('adapter without flag uses loadGame path', async () => {
    const bridge = createMockBridge();
    const resolver = new InMemoryArtifactResolver(testArtifacts);
    const adapter = new PackageRuntimeAdapter(bridge, resolver, { tagNativeLoading: false });
