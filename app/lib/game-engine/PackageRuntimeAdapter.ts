...
import { TAG_GROUPS, DEFAULT_PACKAGE_RUNTIME_FLAGS } from '@slopcade/shared';
...
export class PackageRuntimeAdapter {
  ...
  constructor(bridge: GodotBridge, resolver: ArtifactResolver, flags?: Partial<PackageRuntimeFlags>) {
    this.bridge = bridge;
  ...
  private ensureOrchestrator(): PackageRuntimeOrchestrator {
    if (!this.orchestrator) {
      this.orchestrator = new PackageRuntimeOrchestrator(this.bridge, this.resolver);
    }
  ...
  async loadByTag(tag: TagGroup): Promise<LoadResult> {
    ...
    if (!this.state.manifest) {
      return {
        success: false,
        loadedTags: [],
        errors: [{ code: 'MANIFEST_INVALID', message: 'No manifest loaded. Call loadPackage first.' }],
        durationMs: Date.now() - startTime,
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        loadedTags: [],
        errors: [{ code: 'ARTIFACT_NOT_FOUND', message: err instanceof Error ? err.message : String(err), tag }],
        durationMs: Date.now() - startTime,
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        loadedTags: [tag],
        errors: [{ code: 'BRIDGE_ERROR', message: err instanceof Error ? err.message : String(err) }],
        durationMs: Date.now() - startTime,
      };
    }
  ...
  async reloadChangedTags(manifest: BuildManifest): Promise<LoadResult> {
    if (this.flags.tagNativeLoading) {
      const result = await this.ensureOrchestrator().reloadChangedTags(manifest);
      this.syncStateFromOrchestrator();
      return result;
    }
    ...
    if (changedTags.length === 0) {
      return { success: true, loadedTags: [], durationMs: Date.now() - startTime };
    }
  ...
  private artifactsToGameDefinition(manifest: BuildManifest): GameDefinition {
    const worldPayload = this.resolvedArtifacts.world as TagPayloads['world'] | undefined;
    const prefabsPayload = this.resolvedArtifacts.prefabs as TagPayloads['prefabs'] | undefined;
    const entitiesPayload = this.resolvedArtifacts.entities as TagPayloads['entities'] | undefined;
    const rulesPayload = this.resolvedArtifacts.rules as TagPayloads['rules'] | undefined;
    const scriptsPayload = this.resolvedArtifacts.scripts as TagPayloads['scripts'] | undefined;
  ...
  private getAssetUrls(): string[] {
    const assetsPayload = this.resolvedArtifacts.assets as TagPayloads['assets'] | undefined;
