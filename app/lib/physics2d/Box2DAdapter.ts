...
export class Box2DAdapter implements Physics2D {
  ...
  createMouseJoint(def: MouseJointDef): JointId {
    ...
    const jointDef = this.box2d.b2MouseJointDef();
    jointDef.bodyA = groundBody;
    jointDef.bodyB = bodyRecord.body;
    jointDef.target = this.box2d.b2Vec2(def.target.x, def.target.y);
    ...
    this.joints.set(id, { joint, type: 'mouse' });
    return createJointId(id);
  }
  ...
  setMouseTarget(id: JointId, target: Vec2): void {
    ...
    (record.joint as any).SetTarget?.(t);
  }
