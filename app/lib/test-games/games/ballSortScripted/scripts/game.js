...
// Constants (resolved from bundle constants)
var TUBE_HEIGHT = 6;
var BALL_RADIUS = 0.6;
var BALL_SPACING = 1.32;
var TUBE_BOTTOM_PADDING = 0.18;
var LIFT_OFFSET = 5.0;
...
function calculateDropPosition(ctx, tubeIndex, slot) {
  ...
  var y = tubeY - TUBE_HEIGHT / 2 + TUBE_BOTTOM_PADDING + BALL_RADIUS + slot * BALL_SPACING;
  return { x: tubeX, y: y };
}
...
exports.onPickup = function(ctx, args) {
  ...
  var tubeData = ctx.getEntityData('tube-' + tubeIndex);
  if (tubeData) {
    var liftY = tubeData.position.y + LIFT_OFFSET;
