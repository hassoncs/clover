...
const OUTPUT_DIR = path.join(process.cwd(), 'debug-output/ui-button-test');

async function main() {
  console.log('='.repeat(80));
  console.log('UI Button Generation Test');
  console.log('='.repeat(80));
  ...
  if (!apiKey || !apiSecret) {
    ...
    console.error('  hush run -- npx tsx api/scripts/generate-ui-button.ts');
    process.exit(1);
  }
  ...
  const run: AssetRun<UIComponentSheetSpec> = {
    ...
    meta: {
      ...
      theme: THEME,
      style: 'flat',
      r2Prefix: 'generated/test-ui/button',
      startedAt: Date.now(),
  ...
  try {
    ...
    console.log('\nStage 2: Generating variation states (hover, pressed)...');
...
function generateReport(outputDir: string, run: AssetRun, theme: string) {
  ...
  let report = `# UI Button Generation Report
...
`;
  ...
  if (run.artifacts.stateImages) {
    ...
    report += `✅ **${states.length}/3 states generated successfully**\n\n`;
...
<details>
<summary><strong>Prompt</strong> (${positive.length} chars)</summary>
...
`;
  ...
  const stateOrder = ['normal', 'hover', 'pressed'];
