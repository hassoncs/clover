...
import {
	...
	validateGame,
} from "../src/validation/gameValidator";
...
async function main() {
	...
	if (!existsSync(jsonPath)) {
		...
		console.error(
			"   Run this first: pnpm --filter @slopcade/app tsx scripts/export-test-games.ts",
		);
		process.exit(1);
	}
	...
	for (const template of templates) {
		try {
			...
			try {
				execSync(
					`npx wrangler r2 object put slopcade-assets-dev/${r2Prefix}/definition.json --file=${definitionFile} --content-type=application/json`,
					{
						stdio: "pipe",
						cwd,
					},
				...
				execSync(
					`npx wrangler r2 object put slopcade-assets-dev/${r2Prefix}/metadata.json --file=${metadataFile} --content-type=application/json`,
					{
						stdio: "pipe",
						cwd,
					},
