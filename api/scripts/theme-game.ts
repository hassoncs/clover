...
async function main() {
	const argv = await yargs(hideBin(process.argv))
		...
		.option("style", {
			type: "string",
			description:
				"Style override (presets: pixel, cartoon, 3d, flat, sketch, photorealistic, watercolor, low-poly, voxel, retro — or any custom text)",
		})
		.option("process", {
			type: "boolean",
			default: false,
		...
		.option("production", {
			type: "boolean",
			description:
				"Use production API (https://slopcade-api.hassoncs.workers.dev)",
		})
		.option("dry-run", {
			type: "boolean",
		...
		.check((argv) => {
			...
			if (argv.template && !GAME_IDS.includes(argv.template)) {
				throw new Error(
					`Error: Unknown template '${argv.template}'. Available: ${GAME_IDS.join(", ")}`,
				);
			}
			return true;
		})
	...
	if (argv.process && !argv["dry-run"]) {
		...
		if (!hasScenarioCredentials) {
			...
			console.error(
				`  hush run -- npx tsx api/scripts/theme-game.ts ${process.argv.slice(2).join(" ")}`,
			);
			console.error("═".repeat(60));
			process.exit(1);
		}
	...
	if (authToken === "dev-token" && !argv.local) {
		console.warn(
			"Warning: Using dev-token with non-local API. This will likely fail.",
		);
	...
	if (argv.template) {
		...
		console.log(
			`   Templates: ${Object.keys(gameEntry.definition.templates ?? {}).length}`,
		);
		...
		if (argv["dry-run"]) {
			...
			console.log(
				`Definition size: ${JSON.stringify(gameEntry.definition).length} bytes`,
			);
			console.log("═".repeat(60));
			return;
		}
		...
		try {
			...
			const templateResult = syncResult.results.find(
				(r) => r.id === argv.template,
			);
	...
	try {
		...
		console.log(`Remix ID:   ${result.remixId}`);
		...
		if (argv.process) {
			...
			try {
				const processResult =
					await client.assetSystem.processGenerationJob.mutate({
						jobId: result.jobId,
					});
			...
			console.log(`\nNext steps:`);
			console.log(
				`1. Monitor job status: npx tsx api/scripts/get-job-status.ts ${result.jobId}`,
			);
			console.log(
				`2. Process the job:    npx tsx api/scripts/process-job.ts ${result.jobId}`,
			);
