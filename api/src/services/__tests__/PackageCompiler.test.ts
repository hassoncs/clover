...
import { PackageCompiler, parseWorkspace, buildTagPayloads, contentHash } from '../PackageCompiler';
import type { R2WorkspaceReader, WorkspaceFile, WorkspaceReadResult } from '../R2WorkspaceReader';
import type { BuildArtifactWriter, WriteBuildResult } from '../BuildArtifactWriter';
import type { TagGroup } from '@slopcade/shared';

function createMockReader(files: WorkspaceFile[], errors: string[] = []): R2WorkspaceReader {
  return {
    ...
    readAllFiles: vi.fn().mockResolvedValue({ files, errors } satisfies WorkspaceReadResult),
  } as unknown as R2WorkspaceReader;
}
...
describe('PackageCompiler', () => {
  ...
  describe('compile', () => {
    ...
    it('writes all tag group artifacts to R2', async () => {
      ...
      const [gameId, buildId, manifest, artifacts] = writer.writeBuild.mock.calls[0];
    ...
    it('reports R2 read errors as diagnostics', async () => {
      const reader = createMockReader(MINIMAL_WORKSPACE, ['Failed to read some-file.json: timeout']);
      const compiler = new PackageCompiler(reader, writer);
  ...
  describe('parseWorkspace', () => {
    it('parses all workspace file types', () => {
      ...
      expect(parsed.assetUrls['ball-sprite']).toBe('https://example.com/ball.png');
    });

    it('handles script.js as single entry point', () => {
      const files: WorkspaceFile[] = [
        { path: 'script.js', content: 'exports.onTick = function() { return 1; };' },
      ];
    ...
    it('concatenates multiple script files alphabetically', () => {
      const files: WorkspaceFile[] = [
        { path: 'scripts/b-utils.js', content: 'exports.helper = function() {};' },
        { path: 'scripts/a-main.js', content: 'exports.onTick = function() {};' },
      ];
      const parsed = parseWorkspace(files);
    ...
    it('handles multiple prefab files', () => {
      const files: WorkspaceFile[] = [
        { path: 'prefabs/ball.json', content: JSON.stringify({ id: 'ball', name: 'Ball' }) },
        { path: 'prefabs/wall.json', content: JSON.stringify({ id: 'wall', name: 'Wall' }) },
      ];
      const parsed = parseWorkspace(files);
    ...
    it('warns on duplicate prefab IDs', () => {
      const files: WorkspaceFile[] = [
        { path: 'prefabs/a.json', content: JSON.stringify({ id: 'dup', name: 'First' }) },
        { path: 'prefabs/b.json', content: JSON.stringify({ id: 'dup', name: 'Second' }) },
      ];
      const parsed = parseWorkspace(files);
    ...
    it('provides default world config when world.json is missing', () => {
      ...
      const worldPayload = payloads.get('world') as { world: Record<string, unknown> };
