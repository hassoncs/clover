...
import { WORKSPACE_CONVENTIONS, TAG_GROUPS } from '@slopcade/shared';
import type { R2WorkspaceReader, WorkspaceFile } from './R2WorkspaceReader';
...
function tryParseJson(content: string, filePath: string): { data: unknown; error?: CompileDiagnostic } {
  try {
...
function parseWorkspace(files: WorkspaceFile[]): ParsedWorkspace {
  ...
  if (manifestContent) {
    ...
    const result = tryParseJson(manifestContent, WORKSPACE_CONVENTIONS.manifest);
    if (result.error) {
  ...
  if (entitiesContent) {
    ...
    const result = tryParseJson(entitiesContent, WORKSPACE_CONVENTIONS.entities);
    if (result.error) {
      diagnostics.push(result.error);
    } else {
  ...
  if (assetsJsonContent) {
    ...
    if (result.error) {
      ...
      const parsed = result.data as Record<string, { url?: string; remoteUrl?: string; path?: string }>;
...
function buildTagPayloads(parsed: ParsedWorkspace): Map<TagGroup, unknown> {
  ...
  const worldPayload: TagPayloads['world'] = {
    world: parsed.world?.world ?? { gravity: { x: 0, y: 10 }, pixelsPerMeter: 50 },
...
export class PackageCompiler {
  constructor(
    private readonly reader: R2WorkspaceReader,
  ...
  async compile(gameId: string): Promise<CompileResult> {
    ...
    try {
      await this.writer.writeBuild(gameId, buildId, buildManifest, artifactData);
    } catch (err) {
      allDiagnostics.push({
        severity: 'error',
