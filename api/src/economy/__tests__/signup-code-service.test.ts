...
describe("SignupCodeService", () => {
	...
	describe("validateCode", () => {
		...
		it("throws InvalidSignupCodeError for inactive code", async () => {
			...
			await expect(signupCodeService.validateCode("INACTIVE")).rejects.toThrow(
				InvalidSignupCodeError,
			);
		});
		...
		it("throws InvalidSignupCodeError for expired code", async () => {
			...
			await expect(signupCodeService.validateCode("EXPIRED")).rejects.toThrow(
				InvalidSignupCodeError,
			);
		});

		it("returns valid code info even when max uses reached", async () => {
			await insertSignupCode({
				code: "MAXEDOUT",
			...
			const result = await signupCodeService.validateCode("MAXEDOUT");

			expect(result.valid).toBe(true);
			expect(result.grantAmountMicros).toBe(1000000);
		});
	...
	describe("redeemCode", () => {
		...
		it("increments current uses count after redemption", async () => {
			...
			const code = await env.DB.prepare(
				"SELECT current_uses FROM signup_codes WHERE code = ?",
			)
				.bind("BONUS")
				.first();
		...
		it("records redemption in signup_code_redemptions table", async () => {
			...
			await signupCodeService.redeemCode(
				"RECORDTEST",
				TEST_USER.id,
				walletService,
			);
			...
			const redemption = await env.DB.prepare(
				"SELECT * FROM signup_code_redemptions WHERE user_id = ?",
			)
				.bind(TEST_USER.id)
				.first();
