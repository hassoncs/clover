import type {
	CreditTransaction,
	UserWallet,
} from "@slopcade/shared/schema/economy";
...
export class InsufficientBalanceError extends Error {
	constructor(
		...
		super(
			`Insufficient balance: required ${requiredMicros}, available ${availableMicros}`,
		);
...
export class WalletService {
	...
	async getOrCreateWallet(userId: string): Promise<UserWallet> {
      ...
      VALUES (?, ?, ?, 0, ?, ?)
    `)
			.bind(userId, initialBalance, initialBalance, now, now)
			.run();
