...
type ErrorResponse =
	| {
			success: false;
			error: { type: "validation"; errors: TextGridValidationError[] };
	  }
...
function validationError(
	...
	message: string,
	code = "MISSING_FIELD",
): TextGridValidationError {
	return { code, message, field };
}

function parseTextGridSpec(
	body: unknown,
):
	| { ok: true; spec: TextGridSpec }
	| { ok: false; errors: TextGridValidationError[] } {
	const candidate = isRecord(body) && isRecord(body.spec) ? body.spec : body;
	if (!isRecord(candidate)) {
		return {
			ok: false,
			errors: [validationError("", "Request body must be an object")],
		};
	}
	...
	if (candidate.type !== "text_grid")
		errors.push(validationError("type", "type must be 'text_grid'"));
	if (typeof candidate.id !== "string" || candidate.id.trim().length === 0)
		errors.push(validationError("id", "id must be a non-empty string"));
	if (typeof candidate.text !== "string")
		errors.push(validationError("text", "text must be a string"));
	...
	if (!isRecord(gridRaw)) {
		...
		for (const k of [
			...
			if (typeof gridRaw[k] !== "number")
				errors.push(validationError(`grid.${k}`, `grid.${k} must be a number`));
		}
		if (!isOneOf(gridRaw.align, ["left", "center", "right"] as const)) {
			errors.push(
				validationError(
					"grid.align",
					"grid.align must be one of 'left' | 'center' | 'right'",
				),
			);
		}
	}
	...
	if (!isRecord(wrapRaw)) {
		...
		if (!isOneOf(wrapRaw.mode, ["word", "char"] as const)) {
			errors.push(
				validationError(
					"wrap.mode",
					"wrap.mode must be one of 'word' | 'char'",
				),
			);
		}
		if (
			!isOneOf(wrapRaw.overflow, ["truncate", "ellipsis", "error"] as const)
		) {
			errors.push(
				validationError(
					"wrap.overflow",
					"wrap.overflow must be one of 'truncate' | 'ellipsis' | 'error'",
				),
			);
		}
	}
	...
	if (!isRecord(fontRaw)) {
		...
		for (const k of ["family", "weight", "style"] as const) {
			if (typeof fontRaw[k] !== "string")
				errors.push(validationError(`font.${k}`, `font.${k} must be a string`));
		}
		if (typeof fontRaw.size !== "number")
			errors.push(validationError("font.size", "font.size must be a number"));
	}
	...
	if (!isRecord(silhouetteRaw)) {
		...
		if (typeof silhouetteRaw.mode !== "string")
			errors.push(
				validationError("silhouette.mode", "silhouette.mode must be a string"),
			);
		if (typeof silhouetteRaw.padPx !== "number")
			errors.push(
				validationError(
					"silhouette.padPx",
					"silhouette.padPx must be a number",
				),
			);
		if (typeof silhouetteRaw.fillColor !== "string")
			errors.push(
				validationError(
					"silhouette.fillColor",
					"silhouette.fillColor must be a string",
				),
			);
	}
	...
	if (!isRecord(styleRaw)) {
		...
		style = styleRaw;
		if (typeof styleRaw.prompt !== "string")
			errors.push(
				validationError("style.prompt", "style.prompt must be a string"),
			);
	}
	...
	if (!isRecord(outputRaw)) {
		...
		output = outputRaw;
		if (typeof outputRaw.svg !== "boolean")
			errors.push(
				validationError("output.svg", "output.svg must be a boolean"),
			);
	}
	...
	const spec: TextGridSpec = {
		...
		silhouette: {
			...
			...(typeof silhouette.strokePx === "number"
				? { strokePx: silhouette.strokePx }
				: {}),
			...(typeof silhouette.cornerRoundPx === "number"
				? { cornerRoundPx: silhouette.cornerRoundPx }
				: {}),
			...(typeof silhouette.strokeColor === "string"
				? { strokeColor: silhouette.strokeColor }
				: {}),
		},
		style: {
			...
			style.palette.every((p: unknown) => typeof p === "string")
				? { palette: style.palette as string[] }
				: {}),
		},
		output: {
			svg: output.svg as boolean,
			...(typeof output.rasterFormat === "string"
				? { rasterFormat: output.rasterFormat }
				: {}),
			...(typeof output.rasterScale === "number"
				? { rasterScale: output.rasterScale }
				: {}),
		},
	};
	...
	if (!specValidation.valid)
		return { ok: false, errors: specValidation.errors };
	...
	try {
		...
		try {
			...
			const res: ErrorResponse = {
				...
				error: {
					type: "validation",
					errors: [validationError("", "Invalid JSON", "INVALID_JSON")],
				},
			};
		...
		if (!parsed.ok) {
			const res: ErrorResponse = {
				success: false,
				error: { type: "validation", errors: parsed.errors },
			};
			return c.json(res, 400);
		}
		...
		if (!result.success) {
			if (result.validationErrors?.length) {
				const res: ErrorResponse = {
					success: false,
					error: { type: "validation", errors: result.validationErrors },
				};
				return c.json(res, 400);
			}
			...
			const res: ErrorResponse = {
				...
				error: {
					type: "server",
					message: result.error ?? "Text-grid generation failed",
				},
			};
...
router.post("/stylize", async (c) => {
	...
	if (!token) {
		return c.json(
			{
				success: false,
				error: { type: "server", message: "Authentication required" },
			},
			401,
		);
	}
	...
	if (!c.env.SUPABASE_URL || !c.env.SUPABASE_SERVICE_ROLE_KEY) {
		return c.json(
			{
				success: false,
				error: { type: "server", message: "Auth not configured" },
			},
			500,
		);
	}
	...
	if (authError || !user) {
		return c.json(
			{
				success: false,
				error: { type: "server", message: "Invalid or expired token" },
			},
			401,
		);
	}
	...
	try {
		...
		if (!svg || typeof svg !== "string") {
			return c.json(
				{
					success: false,
					error: { type: "validation", message: "SVG is required" },
				},
				400,
			);
		}
		...
		if (!prompt || typeof prompt !== "string") {
			return c.json(
				{
					success: false,
					error: { type: "validation", message: "Prompt is required" },
				},
				400,
			);
		}
		...
		const estimatedCostMicros = USER_COSTS.ASSET_ENTITY;
		const stylizeId = crypto.randomUUID();
		...
		try {
			await walletService.debit({
				...
				referenceType: "text_grid_stylize",
				referenceId: stylizeId,
				idempotencyKey: `stylize_${stylizeId}`,
				description: "Text grid stylization",
			});
		} catch (err) {
		...
		if (
			...
			await walletService.credit({
				...
				referenceType: "text_grid_stylize",
				referenceId: stylizeId,
				idempotencyKey: `stylize_refund_${stylizeId}`,
				description: "Refund: provider not configured",
			});
			return c.json(
				{
					success: false,
		...
		let uploadResult: { assetId: string };
		try {
			uploadResult = await provider.uploadImage(
				pngBuffer,
				"text-grid-silhouette.png",
			);
		} catch (uploadErr) {
			await walletService.credit({
				userId,
				...
				referenceId: stylizeId,
				idempotencyKey: `stylize_refund_${stylizeId}`,
				description: "Refund: image upload failed",
			});
			throw uploadErr;
		}

		let img2imgResult: { assetId: string };
		try {
			img2imgResult = await provider.img2img({
			...
			await walletService.credit({
				...
				referenceId: stylizeId,
				idempotencyKey: `stylize_refund_${stylizeId}`,
				description: "Refund: img2img failed",
			});
			throw img2imgErr;
		}

		const { buffer: stylizedBuffer, mimeType } = await provider.downloadImage(
			img2imgResult.assetId,
		);
