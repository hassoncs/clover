...
import {
	createFileDebugSink,
	createNodeAdapters,
} from "@/ai/pipeline/adapters/node";
import {
	uiBaseStateStage,
	uiUploadR2Stage,
	uiVariationStatesStage,
} from "@/ai/pipeline/stages/ui-component";
...
const ALL_CONTROLS: UIControlType[] = [
	"button",
	"checkbox",
	"panel",
	"progress_bar",
...
async function generateControl(
	...
	if (variationStates.length > 0) {
		console.log(
			`  Stage 2/3: Generating variations (${variationStates.join(", ")})...`,
		);
...
async function main(): Promise<void> {
	...
	for (const control of controls) {
		...
		try {
			...
			console.error(
				`[FAIL] ${control}: ${error instanceof Error ? error.message : error}`,
			);
			failCount++;
		}
	}
