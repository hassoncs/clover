...
import {
	...
	PromoCodeService,
} from "@/economy/promo-code-service";
import {
	InvalidSignupCodeError,
	SignupCodeService,
} from "@/economy/signup-code-service";
import {
...
export const economyRouter = router({
	...
	hasRedeemedSignupCode: protectedProcedure.query(async ({ ctx }) => {
		const existing = await ctx.env.DB.prepare(
			"SELECT id FROM signup_code_redemptions WHERE user_id = ?",
		)
			.bind(ctx.user.id)
			.first();
	...
	estimateCost: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			const game = await ctx.env.DB.prepare(
				"SELECT r2_prefix FROM games WHERE id = ? AND brand_id = ? AND (user_id = ? OR is_public = 1)",
			)
				.bind(input.gameId, ctx.brandId, ctx.user.id)
	...
	getProducts: protectedProcedure.query(async ({ ctx }) => {
		const products = await ctx.env.DB.prepare(
			"SELECT * FROM iap_products WHERE is_active = 1",
		).all<
			typeof import("@slopcade/shared/schema/economy").iapProducts.$inferSelect
		>();
	...
	processPurchase: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const product = await ctx.env.DB.prepare(
				"SELECT * FROM iap_products WHERE id = ?",
			)
			...
			const existing = await ctx.env.DB.prepare(
				"SELECT id FROM iap_purchases WHERE revenuecat_transaction_id = ?",
			)
