...
import { RatingService, RatingValidationError } from "@/social/rating-service";
...
function getRatingService(db: D1) {
...
function getNotificationService(db: D1) {
...
export const socialRouter = router({
	...
	listComments: publicProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			if (userId) {
				const blockedIds = await new BlockService(ctx.env.DB).getBlockedIds(
					userId,
				);
	...
	editComment: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				...
				if (e instanceof CommentNotFoundError)
					throw new TRPCError({ code: "NOT_FOUND", message: e.message });
				if (e instanceof CommentPermissionError)
	...
	rateGame: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				...
				if (e instanceof RatingValidationError)
					throw new TRPCError({ code: "BAD_REQUEST", message: e.message });
				throw e;
			}
		}),
	...
	getUserProfile: publicProcedure
		...
		.query(async ({ ctx, input }) => {
          ...
          FROM games
          WHERE user_id = ? AND is_public = 1 AND deleted_at IS NULL AND brand_id = ?
	...
	feed: publicProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			let blockedFilter = "";
			const binds: (string | number)[] = [ctx.brandId];
        ...
        LIMIT ? OFFSET ?
      `)
				.bind(...binds, input.limit + 1, input.offset)
				.all<{
					id: string;
					title: string;
					description: string | null;
			...
			if (currentUserId && gameIds.length > 0) {
				const [likes, bookmarks, follows] = await Promise.all([
					getCommentService(db).getReactionStatus(
						currentUserId,
						"game",
						gameIds,
					),
