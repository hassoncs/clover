...
describe("Themes Router", () => {
	...
	beforeEach(() => {
		...
		otherUserCtx = createAuthenticatedContext(TEST_USER_2);
	});
	...
	describe("get", () => {
		it("should get a theme by id", async () => {
			...
			expect(theme.promptModifier).toBe(
	...
	describe("list", () => {
		it("should list themes for current user", async () => {
			...
			expect(themes.every((t) => t.creatorUserId === "test-user-id")).toBe(
				true,
			);
		});
		...
		it("should support pagination with offset", async () => {
			...
			const firstPage = await caller.assetSystem.themes.list({
				limit: 2,
				offset: 0,
			});
			const secondPage = await caller.assetSystem.themes.list({
				limit: 2,
				offset: 2,
			});
		...
		it("should support search by promptModifier", async () => {
			...
			expect(themes.some((t) => t.promptModifier.includes("cyberpunk"))).toBe(
				true,
			);
		});
	...
	describe("listPublic", () => {
		it("should list only public themes", async () => {
			...
			await ctx.env.DB.prepare("UPDATE themes SET is_public = 1 WHERE id = ?")
				.bind(privateTheme.id)
				.run();
		...
		it("should support search by name", async () => {
			...
			expect(themes.some((t) => t.name.toLowerCase().includes("space"))).toBe(
				true,
			);
		});
		...
		it("should support search by promptModifier", async () => {
			...
			expect(
				themes.some((t) => t.promptModifier.includes("unique-search-term-xyz")),
			).toBe(true);
		});
	...
	describe("enhancePrompt", () => {
		it("should return enhanced prompt when API is configured", async () => {
			...
			if (!ctx.env.OPENROUTER_API_KEY) {
				console.log(
					"Skipping enhancePrompt test - OPENROUTER_API_KEY not configured",
				);
				return;
			}
		...
		it("should throw error when API is not configured", async () => {
			...
			const originalKey = ctx.env.OPENROUTER_API_KEY;
			ctx.env.OPENROUTER_API_KEY = undefined;
			...
			await expect(
				...
				"AI enhancement is not configured. Please contact support.",
			);

			// Restore API key
			ctx.env.OPENROUTER_API_KEY = originalKey;
		});
