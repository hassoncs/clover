...
export const socialExtraRouter = router({
  ...
  searchUsers: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const result = await db.prepare(
        `SELECT id, display_name, avatar_url FROM users WHERE display_name LIKE ? LIMIT ? OFFSET ?`
      ).bind(pattern, input.limit, input.offset).all<{
      ...
      const users = await Promise.all(rows.map(async (u) => {
        const [gameCountResult, followerCountResult] = await Promise.all([
          db.prepare('SELECT COUNT(*) as count FROM games WHERE user_id = ? AND is_public = 1 AND deleted_at IS NULL')
            .bind(u.id).first<{ count: number }>(),
          db.prepare('SELECT COUNT(*) as count FROM follows WHERE target_id = ? AND target_type = ?')
            .bind(u.id, 'user').first<{ count: number }>(),
        ]);
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (currentUserId) {
        query = `
          ...
          FROM users u
          LEFT JOIN games g ON g.user_id = u.id AND g.is_public = 1 AND g.deleted_at IS NULL
          WHERE u.id != ?
          GROUP BY u.id
          ORDER BY game_count DESC
          LIMIT ?
        `;
        binds = [currentUserId, input.limit];
      } else {
        query = `
          SELECT u.id, u.display_name, u.avatar_url, COUNT(g.id) as game_count
          FROM users u
      ...
      const result = await db.prepare(query).bind(...binds).all<{
        id: string; display_name: string | null; avatar_url: string | null; game_count: number;
      }>();

      const rows = result.results ?? [];

      const users = await Promise.all(rows.map(async (u) => {
        const followerCountResult = await db.prepare(
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [result, totalResult] = await Promise.all([
        db.prepare(`
          ...
          WHERE r.target_type = ? AND r.target_id = ? AND r.reaction_type = 'like'
          ORDER BY r.created_at DESC
          LIMIT ? OFFSET ?
        `).bind(input.targetType, input.targetId, input.limit, input.offset).all<{
          id: string; display_name: string | null; avatar_url: string | null;
        }>(),
        db.prepare(`
        ...
        `).bind(input.targetType, input.targetId).first<{ count: number }>(),
      ]);
    ...
    .query(async ({ ctx, input }) => {
      ...
      const result = await db.prepare(`
        ...
        WHERE r.user_id = ? AND r.target_type = 'game' AND r.reaction_type = 'like'
          AND g.is_public = 1 AND g.deleted_at IS NULL
        ORDER BY r.created_at DESC
        LIMIT ? OFFSET ?
      `).bind(userId, input.limit + 1, input.offset).all<{
