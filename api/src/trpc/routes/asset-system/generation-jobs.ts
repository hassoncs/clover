...
import type { GenerationJobRow, GenerationTaskRow, RemixRow } from "./types";
...
export const generationJobsRouter = router({
	...
	regenerateRemix: protectedProcedure
		.input(
			z.object({
				remixId: z.string(),
				newTheme: z.string(),
		...
		.mutation(async ({ ctx, input }) => {
			...
			const remixGame = await ctx.env.DB.prepare(
				"SELECT user_id FROM games WHERE id = ? AND deleted_at IS NULL",
			)
				.bind(remixRow.base_game_id)
				.first<{ user_id: string }>();

			if (!remixGame || remixGame.user_id !== ctx.user.id) {
				throw new TRPCError({ code: "FORBIDDEN", message: "Access denied" });
			}

			const overrides = remixRow.asset_overrides_json
				? (JSON.parse(remixRow.asset_overrides_json) as Record<string, unknown>)
				: {};
			const prefabIds = Object.keys(overrides);

			if (prefabIds.length === 0) {
				throw new TRPCError({
					code: "BAD_REQUEST",
					message: "Remix has no entries to regenerate",
				});
			}
			...
			await ctx.env.DB.prepare(`UPDATE remixes SET updated_at = ? WHERE id = ?`)
				.bind(now, input.remixId)
				.run();

			try {
				await ctx.env.DB.prepare(
					`INSERT INTO generation_jobs (id, game_id, remix_id, status, style, created_at)
           VALUES (?, ?, ?, 'queued', ?, ?)`,
				)
					.bind(
						jobId,
						remixRow.base_game_id,
						input.remixId,
						input.newStyle,
						now,
					)
				...
				if (plannerEnabled && ctx.env.OPENROUTER_API_KEY && input.newTheme) {
					console.log(
						"[AssetSystem] Theme planner: generating plan for regenerateRemix",
					);
					const plannerInput = buildPlannerInput(
						definition,
						prefabIds,
		...
		.mutation(async ({ ctx, input }) => {
			...
			for (const task of tasksResult.results) {
               ...
               VALUES (?, ?, 'scenario_txt2img', ?, ?, 'generation_task', ?, ?)`,
						...
						if (jobRow.remix_id) {
							const remixOverrideRow = await ctx.env.DB.prepare(
								"SELECT asset_overrides_json FROM remixes WHERE id = ?",
							)
								.bind(jobRow.remix_id)
								.first<{ asset_overrides_json: string | null }>();

							const currentOverrides = remixOverrideRow?.asset_overrides_json
								? (JSON.parse(remixOverrideRow.asset_overrides_json) as Record<
										string,
										{
											assetId?: string;
											assetUrl?: string;
							...
							currentOverrides[task.template_id] = {
								...currentOverrides[task.template_id],
								assetId,
								assetUrl: result.r2Key,
							};

							await ctx.env.DB.prepare(
								"UPDATE remixes SET asset_overrides_json = ?, updated_at = ? WHERE id = ?",
							)
								.bind(
	...
	createSheetGenerationJob: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const { gameId, remixId, sheetSpec } = input;
			const jobId = crypto.randomUUID();
			const taskId = crypto.randomUUID();
			const now = Date.now();

			await ctx.env.DB.prepare(`
        INSERT INTO generation_jobs (id, game_id, remix_id, status, created_at)
        VALUES (?, ?, ?, 'queued', ?)
      `)
				.bind(jobId, gameId, remixId, now)
				.run();
        ...
        VALUES (?, ?, ?, 'queued', ?)
      `)
				.bind(taskId, jobId, sheetSpec.id, now)
				.run();
	...
	regenerateAssets: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const overrides = remixRow.asset_overrides_json
				? (JSON.parse(remixRow.asset_overrides_json) as Record<string, unknown>)
				: {};
			const validPrefabIds = new Set(Object.keys(overrides));
			const invalidPrefabIds = input.prefabIds.filter(
				(id) => !validPrefabIds.has(id),
			);
			if (invalidPrefabIds.length > 0) {
				throw new TRPCError({
					code: "BAD_REQUEST",
					message: `Invalid prefabIds not in remix: ${invalidPrefabIds.join(", ")}`,
				});
			}
           ...
           VALUES (?, ?, ?, 'queued', ?, ?)`,
				...
				if (plannerEnabled && ctx.env.OPENROUTER_API_KEY && input.newTheme) {
					console.log(
						"[AssetSystem] Theme planner: generating plan for regenerateAssets (partial)",
					);

					const existingPlanRow = await ctx.env.DB.prepare(
						`SELECT theme_plan_json FROM generation_jobs
             WHERE remix_id = ? AND theme_plan_json IS NOT NULL
             ORDER BY created_at DESC LIMIT 1`,
					)
						.bind(input.remixId)
						.first<{ theme_plan_json: string }>();

					if (existingPlanRow) {
						try {
							existingPlan = parseThemePlan(
								JSON.parse(existingPlanRow.theme_plan_json),
							);
							console.log(
								"[AssetSystem] Theme planner: loaded existing plan with anchors",
							);
						} catch (parseErr) {
							console.log(
								"[AssetSystem] Theme planner: failed to parse existing plan, proceeding without anchors",
							);
						}
					}
					...
					const newPlan = await generateThemePlan(
						plannerInput,
						ctx.env.OPENROUTER_API_KEY,
					);
					...
					if (newPlan) {
						...
						if (existingPlan) {
							const coherenceCheck = checkPartialPlanCoherence(
								newPlan,
								existingPlan,
								input.prefabIds,
							);
							if (coherenceCheck.warnings.length > 0) {
								console.log(
									"[AssetSystem] Theme planner: coherence warnings:",
									coherenceCheck.warnings,
								);
							}
