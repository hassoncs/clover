...
export const themesRouter = router({
	create: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
         ...
         VALUES (?, ?, ?, ?, 0, ?, ?)`,
				...
				.bind(id, input.name, input.promptModifier, ctx.user.id, now, now)
				.run();
	...
	update: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (result.meta.changes === 0) {
				throw new TRPCError({
					code: "NOT_FOUND",
					message: "Theme not found or not owned by you",
				});
			}
	...
	list: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			let sql =
				"SELECT * FROM themes WHERE creator_user_id = ? AND deleted_at IS NULL";
	...
	listPublic: publicProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			let sql =
				"SELECT * FROM themes WHERE is_public = 1 AND deleted_at IS NULL";
			const params: any[] = [];
