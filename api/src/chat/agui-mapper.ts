...
function mapPartWithState(
	...
	switch (part.type) {
		...
		case "text-delta": {
			const messageId =
				part.id || state.currentMessageId || ctx.generateMessageId();
			return {
				event: {
		...
		case "finish": {
			return {
				// streamText emits 'finish' after each step in multi-step mode.
				// RUN_FINISHED is emitted by the stream handler after all steps complete.
				event: null,
				state,
			};
		}
