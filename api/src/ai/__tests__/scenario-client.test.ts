...
describe("ScenarioClient", () => {
	...
	describe("constructor", () => {
		...
		it("throws error when apiKey is missing", () => {
			expect(
				() => new ScenarioClient({ apiKey: "", apiSecret: "secret" }),
			).toThrow("Scenario API credentials required");
		});

		it("throws error when apiSecret is missing", () => {
			expect(
				() => new ScenarioClient({ apiKey: "key", apiSecret: "" }),
			).toThrow("Scenario API credentials required");
		});

		it("uses default API URL when not provided", () => {
			const client = new ScenarioClient(validConfig);
			expect((client as unknown as { apiUrl: string }).apiUrl).toBe(
				SCENARIO_DEFAULTS.API_URL,
			);
		});
	...
	describe("usesCustomEndpoint", () => {
		const client = new ScenarioImageClient(new ScenarioClient(validConfig));
	...
	describe("createGenerationJob", () => {
		...
		it("throws error when no jobId returned", async () => {
			...
			await expect(
				client.createGenerationJob({ prompt: "test" }),
	...
	describe("pollJobUntilComplete", () => {
		...
		it("throws error on job failure", async () => {
			...
			await expect(client.pollJobUntilComplete("job-fail")).rejects.toThrow(
				"Generation failed",
			);
		});
		...
		it("throws error when no assets generated", async () => {
			...
			await expect(client.pollJobUntilComplete("job-empty")).rejects.toThrow(
				"No assets generated",
			);
		});
	});
	...
	describe("getAssetDetails", () => {
		...
		it("throws error when asset has no URL", async () => {
			...
			await expect(client.getAssetDetails("asset-no-url")).rejects.toThrow(
				"No URL found for asset",
			);
		});
	});
