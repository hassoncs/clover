...
describe('executeAgentStage', () => {
  ...
  it('runs stage, persists checkpoint payload, and returns success shape', async () => {
    const env = createEnv();
    const runner: AgentStageRunner = vi.fn(async (ctx: AgentExecutionStageContext): Promise<AgentExecutionStageResult> => ({
      status: 'succeeded',
    ...
    const putCalls = (env.ASSETS.put as unknown as ReturnType<typeof vi.fn>).mock.calls;
    expect(putCalls.length).toBeGreaterThan(0);
    expect(putCalls.some((call) => String(call[0]).includes('checkpoint.json'))).toBe(true);
  });
});
