...
const DEBUG_ASSETS: DebugAssetsLevel =
	(process.env.DEBUG_ASSETS as DebugAssetsLevel) || "normal";
const LEVEL_PRIORITY: Record<DebugAssetsLevel, number> = {
	verbose: 0,
	normal: 1,
	quiet: 2,
};
...
function describeComposition(
	...
	const base =
		"The object FILLS THE ENTIRE FRAME. No empty space around it. The edges of the object touch the edges of the image.";
...
export function buildStructuredPrompt(params: StructuredPromptParams): string {
	...
	const shapeDesc = describeShapeSilhouette(
		physicsShape,
		physicsWidth,
		physicsHeight,
	);
	...
	const lines = [
		...
		entityType === "platform"
			? "This is a solid surface that game characters stand on."
			: "",
		entityType === "item" ? "This is a collectible object in the game." : "",
		entityType === "character"
			? "This is a playable character sprite, shown in idle pose."
			: "",
		entityType === "enemy"
			? "This is an enemy character, shown in threatening pose."
			: "",
		...
		downloadImage: async (assetId: string) => {
			...
			const mimeType =
				result.extension === ".png"
					? "image/png"
					: result.extension === ".jpg" || result.extension === ".jpeg"
						? "image/jpeg"
						: "image/webp";
...
export class AssetService {
	...
	selectModel(entityType: EntityType, animated: boolean): string {
		void entityType;
		void animated;
	...
	async generateDirect(
		...
		try {
			...
			const physicsShape: "box" | "circle" =
				entityType === "item" ? "circle" : "box";
			const silhouetteData = createSilhouettePng(physicsShape, width, height);

			assetLog(
				"DEBUG",
				"",
				`Physics: shape=${physicsShape}, width=${width}, height=${height}`,
			);
			assetLog(
				"INFO",
				"",
				`Silhouette created: 512x512 for ${physicsShape} ${width}x${height}`,
			);
			...
			const silhouetteR2Key = await this.uploadToR2(
				silhouetteData,
				".png",
				entityType,
				context,
			...
			assetLog(
				"INFO",
				"",
				`Uploaded silhouette to provider: ${uploadedAssetId}`,
			);
			assetLog(
				"INFO",
				"",
				`Starting img2img generation with strength=${strength}`,
			);
			...
			await this.saveDebugFile(
				`${debugId}_metadata.json`,
				JSON.stringify(
					{
						debugId,
	...
	async generateFromStructuredParams(
		params: StructuredPromptParams,
	): Promise<AssetGenerationResult> {
	...
	async generateWithSilhouette(
		...
		try {
			...
			const errorMessage = err instanceof Error ? err.message : "Unknown error";
			console.error(
				`[AssetService] Silhouette generation failed: ${errorMessage}`,
			);
	...
	async generateAsset(
		...
		const { entityType, description, size, context } = request;
		...
		try {
			...
			console.log(
				`[AssetService] Creating silhouette for ${entityType} (${width}x${height}, ${physicsShape})`,
			);

			const uploadedAssetId = await provider.uploadImage(silhouetteData);

			console.log(
				`[AssetService] Generating ${entityType} with silhouette img2img`,
			);
	...
	async removeBackground(
		...
		try {
			...
			const uploadedAssetId = await provider.uploadImage(
