...
export const STYLE_PRESETS: Record<string, string> = {
	...
	cartoon:
		"cartoon illustration, bold outlines, vibrant flat colors, cel-shaded",
	...
	photorealistic:
		"photorealistic, highly detailed, natural lighting, physically accurate materials",
	watercolor:
		"watercolor painting, soft bleeding edges, translucent washes, paper texture",
...
export interface AssetRun<T extends AssetSpec = AssetSpec> {
	...
	meta: {
		gameId: string;
		/** UUID grouping this run's assets in R2 */
		remixId: string;
		...
		theme: string;
		style?: string;
		/** @deprecated Use buildAssetPath(gameId, remixId, assetId) */
		r2Prefix: string;
		startedAt: number;
		runId: string;
...
export function isUIComponentSpec(
	spec: AssetSpec,
): spec is UIComponentSheetSpec {
	return (
		spec.type === "sheet" && "kind" in spec && spec.kind === "ui_component"
	);
}
