...
function calculateStrengthFromColor(
	...
	return (
		MIN_DYNAMIC_STRENGTH +
		(1 - luminance) * (MAX_DYNAMIC_STRENGTH - MIN_DYNAMIC_STRENGTH)
	);
}
...
function calculateSheetDimensions(layout: SheetLayout): {
	width: number;
	height: number;
} {
	if (layout.type === "grid") {
		...
		for (let i = 0; i < layout.frameCount; i++) {
			const x =
				layout.direction === "horizontal"
					? margin + i * (layout.cellWidth + spacing)
					: margin;
			const y =
				layout.direction === "vertical"
					? margin + i * (layout.cellHeight + spacing)
					: margin;
			entries.push({
				index: i,
				x,
...
export const silhouetteStage: Stage = {
	...
	async run(
		...
		if (entitySpec.skipSilhouette) {
			console.log(
				`  [silhouette] Skipping silhouette for ${entitySpec.id} (skipSilhouette=true, will use txt2img)`,
			);
			return run;
		}
...
export const buildPromptStage: Stage = {
	...
	async run(
		run: AssetRun,
		_adapters: PipelineAdapters,
		debug: DebugSink,
	): Promise<AssetRun> {
		const { prompt } = buildPromptForSpec(run.spec, {
			theme: run.meta.theme,
			style: run.meta.style,
		});
...
export const uploadToProviderStage: Stage = {
	...
	async run(
		...
		const imageToUpload =
			run.artifacts.silhouettePng ?? run.artifacts.sheetGuidePng;
		if (!imageToUpload) {
			console.log(
				`  [upload-provider] No silhouette/guide image — skipping upload (will use txt2img)`,
			);
			return run;
		}
...
export const img2imgStage: Stage = {
	...
	async run(
		...
		if (!imageAssetId) {
			console.log(
				`  [img2img] No silhouette uploaded — falling back to txt2img`,
			);
			const entitySpec =
				run.spec.type === "entity" ? (run.spec as EntitySpec) : null;
		...
		const entityColor =
			run.spec.type === "entity" ? (run.spec as EntitySpec).color : undefined;
		const colorBasedStrength = calculateStrengthFromColor(entityColor);
		const strength =
			run.meta.strength ?? colorBasedStrength ?? DEFAULT_IMG2IMG_STRENGTH;
...
export const txt2imgStage: Stage = {
	...
	async run(
		...
		const height =
			"height" in spec && spec.height ? spec.height : defaultHeight;
...
export const layeredDecomposeStage: Stage = {
	...
	async run(
		...
		for (let i = 0; i < result.assetIds.length; i++) {
			const { buffer } = await adapters.provider.downloadImage(
				result.assetIds[i],
			);
...
export const uploadR2Stage: Stage = {
	...
	async run(
		...
		const getFinalImage = (): Uint8Array | null => {
			return (
				run.artifacts.bgRemovedImage ?? run.artifacts.generatedImage ?? null
			);
		};
		...
		if (run.spec.type === "parallax" && run.artifacts.layerImages) {
			...
			if (run.artifacts.sheetMetadataJson && run.spec.type === "sheet") {
				...
				const metadataBuffer = new Uint8Array(
					Buffer.from(run.artifacts.sheetMetadataJson),
				);
...
export const sheetGuideStage: Stage = {
	...
	async run(
		...
		const spec = run.spec;
		const { width: gridWidth, height: gridHeight } = calculateSheetDimensions(
			spec.layout,
		);
		...
		const scale =
			Math.min(CANVAS_SIZE / gridWidth, CANVAS_SIZE / gridHeight) * 0.9;
