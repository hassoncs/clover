 ...
 */
export function createNodeScenarioAdapter(config: {
	apiKey: string;
	apiSecret: string;
}): ImageGenerationAdapter {
...
export async function createNodeSilhouetteAdapter(): Promise<SilhouetteAdapter> {
	...
	return {
		async createSilhouette(params): Promise<Uint8Array> {
			...
			const strokeWidth = needsStroke
				? Math.max(4, Math.floor(minDimension * 0.3))
				: 0;
			const strokeAttr = needsStroke
				? `stroke="#333333" stroke-width="${strokeWidth}"`
				: "";
...
export function createNodeModalAdapter(
	...
	async function waitForReady(): Promise<void> {
		...
		throw new Error(
			`Modal failed to become ready after ${maxWakeTimeMs / 1000}s`,
		);
	}
 ...
 * providing the same interface used by the rest of the codebase.
 */
export function createNodeComfyUIAdapter(
...
export async function createNodeAdapters(
	options: NodeAdaptersOptions,
	...
	if (provider === "scenario") {
		...
		if (!apiKey || !apiSecret) {
			throw new Error(
				"SCENARIO_API_KEY and SCENARIO_SECRET_API_KEY required when using Scenario provider. Use `hush run -- <command>` to inject secrets from the hush vault.",
			);
		}
...
export function createFileDebugSink(outputDir: string): DebugSink {
	...
	return async (event: DebugEvent) => {
		if (event.type === "artifact") {
			...
			console.log(
				`  [STAGE] ${event.stageId} ${status} (${event.durationMs}ms)${event.error ? ` - ${event.error}` : ""}`,
			);
