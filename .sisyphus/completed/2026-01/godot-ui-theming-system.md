...
- **Scope**: Extend existing Button/CheckBox POC to 4 additional controls:
  ...
  - 🆕 Panel
  - 🆕 ProgressBar
  - 🆕 ScrollBar (Horizontal + Vertical as separate types)
  - 🆕 TabBar
...
**Research Findings**:
- ✅ POC Complete: `.sisyphus/completed/2026-01/ui-component-generation.md`
- ✅ Type system supports `UIComponentSheetSpec` in `api/src/ai/pipeline/types.ts`
- ✅ Pipeline stages: silhouette → img2img (base @ 0.95, variations @ 0.7) → R2 → metadata
- ✅ Godot integration: `ThemedUIComponent.gd` loads textures + creates StyleBoxTexture
- ✅ Bridge: `GameBridge.gd` exposes `createUIButton()` - needs extension
...
**Guardrails Applied**:
- ❌ No animated states (static textures only)
- ❌ No theme persistence/saving (theme is input parameter)
- ❌ No user-configurable margins (hardcoded constants)
- ❌ No regeneration UI (one-shot generation)
- ❌ No composite controls (e.g., Dropdown = Button + Panel)
- ❌ No accessibility features (out of scope)
- ❌ No cross-control consistency enforcement (each generated independently)
- ❌ Only 4 new controls (Panel, ProgressBar, ScrollBar H/V, TabBar)
...
- ❌ Other control types (LineEdit, Tree, ItemList, PopupMenu, Window, Tooltip)
- ❌ Animated state transitions
- ❌ Theme persistence/saving UI
- ❌ Regeneration workflow UI
- ❌ Multiple style variants per theme
- ❌ Composite controls (Dropdown, ComboBox, etc.)
- ❌ User-configurable nine-patch margins
- ❌ Automatic margin detection (edge detection, ML)
- ❌ Cross-control style consistency enforcement
- ❌ Accessibility features (ARIA, screen reader)

**AI-Slop Patterns to Avoid**:
- ❌ Over-abstraction: No `ControlShapeFactory` with plugin system for 6 controls
- ❌ Scope inflation: No additional control types beyond the 6 agreed upon
- ❌ Premature optimization: No caching layer for silhouettes or assets
- ❌ Feature creep: No theme editing UI, no theme gallery, no theme marketplace
- ❌ Over-engineering: No DSL for silhouette shapes, no complex layout system
...
Phase 1: Type System & Constants
  Task 1 (Types) → Task 2 (Constants)

Phase 2: SVG Silhouette Generation
  Task 3 (Panel) ┐
  Task 4 (ProgressBar) ├─→ All parallel (independent)
  Task 5 (ScrollBar) ┤
  Task 6 (TabBar) ┘
  ↓
...
Phase 3: Prompt & Pipeline
  Task 7 (Prompt Builder) → Task 8 (Pipeline Stages)

Phase 4: Godot Integration
  Task 9 (ThemedUIComponent) → Task 10 (GameBridge) → Task 11 (Test Scene)

Phase 5: Batch API & QA
  Task 12 (Batch API) → Task 13 (Manual QA)
```
  ...
  **What to do**:
  ...
  - Implement `createPanelSilhouette()`:
    ...
    - Use SVG path with "fill-rule: evenodd" for hollow effect:
      ...
      ```
    - Convert SVG → PNG buffer using library (research: `sharp` with SVG input, or `resvg-js`)
...
- [ ] 13. End-to-End Manual QA
  ...
  **What to do**:
  - Full workflow test (API → Godot → Visual Verification):
  ...
  - Document findings in `.sisyphus/notepads/ui-theme-qa/qa-results.md`:
    - Screenshots (3 themes × 7 controls = 21 images minimum)
  ...
  **Evidence Required**:
  - [ ] Screenshots: 3 themes × 7 controls × 3 sizes = 63 images minimum
