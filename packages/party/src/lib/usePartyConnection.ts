...
export function usePartyConnection({
	...
	const { getAuthToken, apiUrl, getStorageItem, setStorageItem } = usePartyConfig();
	...
	const connect = useCallback(async () => {
		...
		try {
			...
			if (role === "host" && hostToken) {
				...
				const reconnectToken = playerTokenRef.current ?? (await getStorageItem(getPlayerTokenKey(code)));
				if (reconnectToken) {
			...
			const wsUrl = `${apiUrl.replace(/^http/, "ws")}/api/party/${code}/ws?${params.toString()}`;
				...
				try {
					...
					switch (message.type) {
						...
						case "player_token":
							...
							void setStorageItem(getPlayerTokenKey(code), message.token);
							break;
						case "error":
							console.error(
