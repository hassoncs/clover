...
export default function FallingBoxesV2() {
  ...
  const dragHandlers = useDragInteraction(physicsRef, {
    pixelsPerMeter: PIXELS_PER_METER,
    maxForce: 5000,
    stiffness: 10,
    damping: 0.9,
    onDragStart: (bodyId) => {
      setDraggedBodyId(bodyId.value);
    },
    onDragEnd: () => {
  ...
  const stepPhysics = useCallback((dt: number) => {
    ...
    const updatedBoxes = bodyIdsRef.current.map((bodyId, i) => {
      ...
      return {
        ...
        height: BOX_SIZE * PIXELS_PER_METER,
        color: COLORS[i % COLORS.length],
        isDragging: bodyId.value === draggedBodyId,
      };
    });
  ...
  return (
    ...
    >
      <Canvas ref={canvasRef} style={{ flex: 1 }} pointerEvents="none">
        <Fill color="#1a1a2e" />
