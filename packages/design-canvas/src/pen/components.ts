...
function isReusable(node: PenNode): boolean {
	return (
		(node.type === "frame" || node.type === "ref") &&
		!!(node as PenFrame | PenRef).reusable
	);
}

function walkDepthFirst(
	nodes: PenNode[],
	visitor: (node: PenNode) => void,
): void {
...
export function resolveRef(
	...
	for (const key of VISUAL_OVERRIDE_KEYS) {
		const value = (refNode as Record<string, unknown>)[key];
		if (value !== undefined) {
			(cloned as Record<string, unknown>)[key] = value;
		}
	}
	...
	if (refNode.descendants) {
		for (const [slashPath, patch] of Object.entries(refNode.descendants)) {
			...
			if (target) {
				const { children: patchChildren, ...restPatch } = patch as Record<
					string,
					unknown
				>;
	...
	return nodes.map((node) => {
		if (node.type === "ref") {
			if (resolvingStack.has(node.ref)) {
				console.warn(
					`Circular ref detected: ${node.ref} — skipping resolution`,
				);
				return node;
			}
			...
			if (children) {
				setChildren(
					resolved,
					resolveAllRefsInner(children, registry, resolvingStack),
				);
			}
