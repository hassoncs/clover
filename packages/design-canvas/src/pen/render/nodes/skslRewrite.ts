...
export function rewriteGodotToSkSL(glsl: string): SkSLRewriteResult {
	const warnings: string[] = [];
	const uniforms: SkSLUniform[] = [];
	let result = glsl;

	if (
		result.includes("SCREEN_TEXTURE") ||
	...
	const uniformRegex =
		/^\s*uniform\s+(int|float|vec[234])\s+(\w+)\s*(?::\s*[^=;]*)?\s*(?:=\s*([^;]*?))?\s*;/gm;
	const parsedUniforms: Array<{
		original: string;
		name: string;
		godotType: string;
		defaultVal?: string;
...
function godotTypeToSkSL(godotType: string): string {
...
function parseDefaultValue(raw?: string): number | number[] | undefined {
 ...
 */
function rewriteColorToReturn(source: string): string {
	let result = source;

	result = result.replace(
		/\bCOLOR\s*=\s*vec4\s*\(([^;]*)\)\s*;/g,
		(_, inner) => `return half4(${inner});`,
	);

	result = result.replace(
		/\bCOLOR\s*=\s*(?!vec[234]\s*\()([^;]+);/g,
		(_, expr) => `return half4(${expr.trim()});`,
	);

	return result;
}

export const SKSL_COMPATIBLE_GENERATORS: string[] = [
	"rbAurora",
	"rbBalatro",
	"rbColorBends",
	"rbDarkVeil",
