...
function createServer(): McpServer {
	...
	for (const [name, def] of toolDefinitions) {
		server.tool(name, def.description, def.schema, async (args) => {
			...
			const perCallTargetId = mutableArgs.targetId as string | undefined;
			delete mutableArgs.targetId;
			...
			return currentDef.handler(mutableArgs);
		});
	}
	...
	if (sessionId && transports[sessionId]) {
		...
		transport.onclose = () => {
			if (transport.sessionId) {
				...
				console.error(
					`[game-inspector] Session closed: ${transport.sessionId}`,
				);
			}
		};
...
app.post("/reload", async (_req, res) => {
	...
	res.json({
		...
		toolCount: toolDefinitions.size,
	});
});
...
app.listen(PORT, () => {
	console.error(
		`[game-inspector] HTTP server running on http://localhost:${PORT}/mcp`,
	);
