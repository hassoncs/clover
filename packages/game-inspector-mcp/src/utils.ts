	...
	try {
		await page.waitForFunction(
			() => {
				const w = window as unknown as WindowWithBridge;
				return (
					w.SlopcadeDebugBridge?.ready === true || w.slopcadeGameReady === true
				);
			},
			{ timeout },
		);
...
export async function activateTarget(
	...
	await page.evaluate((id: string) => {
		...
		if (registry && registry[id]) {
			(window as any).debugOps = registry[id];
		}
	}, targetId);
}
...
export function resolveTargetId(
	...
	return argsTargetId ?? state.activeTargetId;
}
...
export async function queryGodot<T>(
	...
	return page.evaluate(
		async (evalArgs: { method: string; args: unknown[] }) => {
			...
			if (!win.debugOps) {
				return {
					error:
						"debugOps not available. Ensure game is loaded with ?debug=true",
				};
			}
...
export async function querySlopcade<T>(
	page: Page | null,
	method: string,
	args: unknown[] = [],
): Promise<T | { error: string }> {
