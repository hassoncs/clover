...
| Type | Schema | Game | Status |
|------|--------|------|--------|
| **QuipPrompt** | `{ id, text, category }` | Quiplash | ✅ Ready |
| **TriviaQuestion** | `{ id, question, correctAnswer, incorrectAnswers[], category, difficulty? }` | Trivia Murder Party | 🔴 Game not built |
| **FibbageQuestion** | `{ id, question, answer, category, alternateSpellings? }` | Fibbage | 🔴 Game not built |
| **DrawingPrompt** | `{ id, prompt, category, difficulty? }` | Drawful | 🔴 Game not built |
| **WouldYouRather** | `{ id, optionA, optionB, category }` | WYR | 🔴 Game not built |
| **EstimationQuestion** | `{ id, question, answer (number), unit?, category, acceptableRange? }` | Estimation | 🔴 Game not built |
| **CaptionPrompt** | `{ id, imageUrl, category, context? }` | Caption game | 🔴 Game not built |
| **WordGamePrompt** | `{ id, type, prompt, category, difficulty? }` | Word games | 🔴 Game not built |
