...
export const LikersBottomSheet = forwardRef<
	LikersBottomSheetHandle,
	{ currentUserId: string | null }
>(function LikersBottomSheet(
	...
	const { data, isLoading } = trpc.socialExtra.getLikers.useQuery(
		{
		...
		>
			...
			<BottomSheetScrollView className="flex-1">
				{isLoading ? (
					...
					users.map((user: any) => (
						<LikerRow key={user.id} currentUserId={currentUserId} user={user} />
					))
				)}
