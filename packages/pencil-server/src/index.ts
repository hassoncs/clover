...
import { parsePenDocument, type PenDocument } from "@slopcade/shared/types/pen";
...
const PORT = 8090;
const CANVAS_FILE = path.join(process.cwd(), "canvas.pen");
...
async function main() {
  ...
  const httpServer = app.listen(PORT, () => {
    console.log(`🔌 Pencil Server running on port ${PORT}`);
  });
