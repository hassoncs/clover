...
export class CameraSystem {
	...
	update(
		dt: number,
		entityGetter?: (id: string) => RuntimeEntity | undefined,
	): void {
	...
	private updateFollow(
		...
		if (this.config.lookAhead?.enabled) {
			...
			this.state.lookAheadOffset = {
				x: lerp(
					this.state.lookAheadOffset.x,
					lookAheadTarget.x,
					lookAheadSmoothing,
				),
				y: lerp(
					this.state.lookAheadOffset.y,
					lookAheadTarget.y,
					lookAheadSmoothing,
				),
			};
		...
		this.state.position = {
			x: followX
				? lerp(this.state.position.x, desiredPos.x, smoothing)
				: this.state.position.x,
			y: followY
				? lerp(this.state.position.y, desiredPos.y, smoothing)
	...
	private updateZoomEffect(dt: number): void {
		...
		this.zoomEffectScale =
			this.zoomEffectStart +
			(this.zoomEffectTarget - this.zoomEffectStart) * eased;
	...
	getWorldToScreenTransform(): CameraTransform {
		...
		const translateX =
			centerX - worldX * this.pixelsPerMeter * this.config.zoom;
		const translateY =
			centerY - worldY * this.pixelsPerMeter * this.config.zoom;
	...
	getViewportWorldBounds(): {
		...
		const halfWidth =
			this.viewport.width / this.pixelsPerMeter / this.config.zoom / 2;
		const halfHeight =
			this.viewport.height / this.pixelsPerMeter / this.config.zoom / 2;
