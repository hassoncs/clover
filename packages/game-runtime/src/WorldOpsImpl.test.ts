...
describe("WorldOpsImpl", () => {
	...
	describe("spawn", () => {
		...
		it("should apply velocity if provided", async () => {
			await worldOps.spawn(
				"ball",
				{ x: 0, y: 0 },
				{ velocity: { x: 5, y: -10 } },
			);

			expect(mockPhysics.setLinearVelocity).toHaveBeenCalledWith("entity_2", {
				x: 5,
				y: -10,
			});
		});
		...
		it("should reparent if parentId provided", async () => {
			...
			expect(mockEntityManager.reparent).toHaveBeenCalledWith(
				"entity_2",
				"parent_1",
			);
		});
	...
	describe("animate", () => {
		it("should return Promise that resolves on tween completion", async () => {
			const promise = worldOps.animate(
				"entity_1",
				{ x: 100, y: 200 },
				{ duration: 1000 },
			);
	...
	describe("tags", () => {
		it("should add tag", async () => {
			...
			expect(mockEntityManager.addTag).toHaveBeenCalledWith(
				"entity_1",
				"new-tag",
			);
		});
		...
		it("should remove tag", async () => {
			...
			expect(mockEntityManager.removeTag).toHaveBeenCalledWith(
				"entity_1",
				"test-tag",
			);
		});
		...
		it("should check if entity has tag", async () => {
			...
			expect(result).toBe(true);
			expect(mockEntityManager.hasTag).toHaveBeenCalledWith(
				"entity_1",
				"test-tag",
			);
		});
	...
	describe("variables", () => {
		...
		it("should set variable and emit event", async () => {
			...
			expect(mockEventQueue.emit).toHaveBeenCalledWith("variable_change", {
				name: "score",
				value: 200,
			});
		});
	...
	describe("game state", () => {
		it("should emit win event", async () => {
			...
			expect(mockEventQueue.emit).toHaveBeenCalledWith("game_state_change", {
				state: "won",
			});
		});
		...
		it("should emit custom event", async () => {
			...
			expect(mockEventQueue.emit).toHaveBeenCalledWith("custom_event", {
				foo: "bar",
			});
		});
	});
