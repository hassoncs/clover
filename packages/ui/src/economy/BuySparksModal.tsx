...
const SPARK_PACKS: SparkPack[] = [
	...
	},
	{
		...
		sparks: 100000,
		price: "$79.99",
		generations: "~1000 assets",
	},
];

export function BuySparksModal({
	visible,
	onClose,
	onRedeemPromo,
}: BuySparksModalProps) {
	...
	const handleRedeemPromo = async () => {
		...
		try {
			...
			setPromoMessage({ text: message, type: "error" });
		} finally {
					...
					<View className="p-4">
								...
								<View className="flex-row items-center justify-between">
									<View className="flex-1">
										...
										<View className="flex-row items-baseline mt-1">
											<Text className="text-theme-warning font-bold text-2xl">
										...
										<Text className="text-theme-text-tertiary text-sm mt-1">
											{pack.generations}
										</Text>
									</View>
						...
						<View className="mt-6 p-4 bg-theme-surface-elevated/30 rounded-xl">
							<Text className="text-theme-text-tertiary text-center text-xs">
								Sparks are used for asset generation costs.
