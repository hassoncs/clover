	...
	const onMouseMove = useCallback(
		(e: any) => {
			...
			if (state.type === "multi-drag") {
				...
				if (!frame) return;
				for (const el of frame.elements) {
					const initEl = state.initialElements[el.id] as any;
					if (!initEl) continue;
					if (el.type === "line") {
						el.x1 = initEl.x1 + dx;
						el.y1 = initEl.y1 + dy;
			...
			if (!state.initialElement || !state.initialFrame) return;
			...
			if (state.type === "drag") {
				...
				const { selX, selY, selW, selH } = getElementBounds(
