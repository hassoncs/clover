...
export function useDesignInteractions(params: {
  ...
  const { document, camera, selectedFrameId, selectedElementId, saveDesignDocument, cameraHandlers } = params;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (!selectedFrameId || !selectedElementId || !document) return;
      ...
      const newDoc = JSON.parse(JSON.stringify(document)) as DesignDocument;
  ...
  const getElementBounds = (element: DesignElement, frame: DesignFrame) => {
  ...
  const hitTestHandles = (worldX: number, worldY: number) => {
    if (!document || !selectedFrameId || !selectedElementId) return null;
    ...
    for (const h of handles) {
      if (Math.abs(worldX - h.x) <= hitRadius && Math.abs(worldY - h.y) <= hitRadius) {
        return { type: "resize" as const, handle: h.id };
      }
    }

    const rotX = selX + selW / 2;
    const rotY = selY - 24;
    if (Math.abs(worldX - rotX) <= hitRadius && Math.abs(worldY - rotY) <= hitRadius) {
  ...
  const onMouseDown = useCallback((e: any) => {
    ...
    if (handleHit) {
      ...
      if (frame && element) {
        ...
        if (handleHit.type === "resize") setIsResizing(true);
        if (handleHit.type === "rotate") setIsRotating(true);
        e.stopPropagation();
        return;
      }
    }
    ...
    cameraHandlers.onMouseDown(e);
  }, [document, camera, selectedFrameId, selectedElementId, cameraHandlers]);
  ...
  const onMouseMove = useCallback((e: any) => {
    ...
    if (!document || !state.initialElement || !state.initialFrame) return;
    ...
    if (state.type === "drag") {
      if (element.type === "line") {
        const initEl = state.initialElement as any;
        ...
        if (!e.shiftKey) {
          ...
          const elW = element.type === "path" ? 40 : (element as any).width;
          const elH = element.type === "path" ? 40 : (element as any).height;
          ...
          const xTargets = [frameX, frameX + frame.width / 2, frameX + frame.width];
          const yTargets = [frameY, frameY + frame.height / 2, frameY + frame.height];
          ...
          for (const target of xTargets) {
            if (Math.abs(worldElX - target) < minXDist) { minXDist = Math.abs(worldElX - target); bestXSnap = { target, offset: 0 }; }
            if (Math.abs(worldElCX - target) < minXDist) { minXDist = Math.abs(worldElCX - target); bestXSnap = { target, offset: elW / 2 }; }
            if (Math.abs(worldElR - target) < minXDist) { minXDist = Math.abs(worldElR - target); bestXSnap = { target, offset: elW }; }
          }

          let bestYSnap = null;
          let minYDist = SNAP_DIST;
          for (const target of yTargets) {
            if (Math.abs(worldElY - target) < minYDist) { minYDist = Math.abs(worldElY - target); bestYSnap = { target, offset: 0 }; }
            if (Math.abs(worldElCY - target) < minYDist) { minYDist = Math.abs(worldElCY - target); bestYSnap = { target, offset: elH / 2 }; }
            if (Math.abs(worldElB - target) < minYDist) { minYDist = Math.abs(worldElB - target); bestYSnap = { target, offset: elH }; }
          }
      ...
      if (element.type !== "line" && element.type !== "path") {
        ...
        if (newW < 4) {
          if (state.handle?.includes("l")) newX -= (4 - newW);
          newW = 4;
        }
        if (newH < 4) {
          if (state.handle?.includes("t")) newY -= (4 - newH);
          newH = 4;
        }

        element.x = newX;
        element.y = newY;
        (element as any).width = newW;
        (element as any).height = newH;
      }
