...
import { compileTypeScriptGame } from './ts-compiler';

export type GameFormat = 'typescript' | 'bundle' | 'unknown';
...
export function detectGameFormat(gamePath: string): GameFormat {
  ...
  if (stat.isFile()) {
    if (gamePath.endsWith('.ts') || gamePath.endsWith('.tsx')) {
      return 'typescript';
    }
    return 'unknown';
  }
  ...
  if (stat.isDirectory()) {
    ...
    const gameTs = join(gamePath, 'game.ts');
...
export function loadGameFromPath(gamePath: string): LoadGameResult {
  ...
  if (format === 'typescript') {
    const tsPath = gamePath.endsWith('.ts') ? gamePath : join(gamePath, 'game.ts');
    const result = compileTypeScriptGame(tsPath);
  ...
  if (format === 'bundle') {
    ...
    const metadata = {
      title: (manifest.title as string) || (manifest.name as string) || basename(gamePath),
