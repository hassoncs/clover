...
import { type FileReader, NodeFileReader } from "./FileReader";
import type {
	BundleCompileResult,
	BundleSections,
	CompileError,
...
function readJsonFile(
	filePath: string,
	fileReader: FileReader,
): { data: unknown; error?: CompileError } | null {
	try {
...
function readScriptFile(
	filePath: string,
	fileReader: FileReader,
): { content: string; error?: CompileError } {
	try {
		const content = fileReader.readFileSync(filePath);
...
function resolveConstantRefs(
	...
	if (Array.isArray(obj)) {
		return obj.map((item, index) =>
			resolveConstantRefs(item, constants, [...path, `[${index}]`], errors),
		);
	}
...
function buildGameDefinition(
	...
	const world: GameDefinition["world"] = {
		gravity: (worldConfig.gravity as { x: number; y: number }) || {
			x: 0,
			y: 10,
		},
...
export function compileBundle(
	...
	for (const filePath of jsonFiles) {
		...
		} else if (relativePath === "effects.json") {
			if (typeof data === "object" && data !== null) {
	...
	if (
		fileReader.existsSync(schemasDir) &&
		fileReader.statSync(schemasDir).isDirectory()
	) {
		rawData.schemas = {};
		...
		for (const schemaFile of schemaFiles) {
			...
			if (fileReader.existsSync(schemaPath)) {
				...
				if (result && result.data && typeof result.data === "object") {
					const schemaName = schemaFile.replace(".json", "") as
						| "level"
						| "persistence";
	...
	const resolvedEntities = rawData.entities.map(
		(e) =>
			resolveConstantRefs(e, constantsMap, ["entities"], errors) as Record<
				string,
				unknown
			>,
	);
