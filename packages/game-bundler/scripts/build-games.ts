...
import { existsSync, mkdirSync, writeFileSync, readdirSync, watch } from 'fs';
...
async function compileGameTypeScript(gamePath: string): Promise<GameDefinition | null> {
...
async function buildAll(): Promise<boolean> {
  ...
  const games = gameEntries
    .filter(e => e.isDirectory() && existsSync(join(GAMES_DIR, e.name, 'game.ts')))
    .map(e => e.name);
