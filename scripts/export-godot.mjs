...
const watchMode =
	args.includes("--watch") || !args.some((a) => a.startsWith("--"));
...
function checkExport() {
	...
	if (outputTime === 0) {
		console.log(
			"Godot export missing - this is usually handled automatically by the watcher in 'pnpm dev'.",
		);
	...
	if (sourceTime > outputTime) {
		console.log(
			"Godot export out of date - the watcher should be running. Check 'devmux status'.",
		);
		console.log(
			"To force a manual rebuild: node scripts/export-godot.mjs --once",
		);
		process.exit(1);
	}
