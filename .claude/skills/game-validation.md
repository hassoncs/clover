...
| `shared/src/validation/semantic.ts` | `validateSemantic()`, `validateEntityPrefabRefs()`, `validateParentChildCycles()`, `validateConstantRefs()` |
...
5. `validatePrefabs()` — validates each prefab's physics, collider, visual
6. `validateEntities()` — validates entity IDs, transforms, prefab refs, components; checks for duplicate IDs
7. `validateSemantic()` — cross-reference validation (see below)

### `validateSemantic(game, errors, warnings): void`

Mutates the passed `errors`/`warnings` arrays. Runs:
1. `validatePrefabReferences()` — checks prefab child refs, entity child refs; detects duplicate prefab IDs
...
4. `validateParentChildCycles()` — separate cycle detection on parent/child relationships
5. Constant reference validation — walks entire game tree for `{ const: "name" }` references
...
3. Extracts expressions and validates each
...
| `NO_WIN_MECHANISM` | no winCondition.expr, no script calling win() |
...
| `UNKNOWN_PREFAB_REFERENCE` | prefab/entity child references unknown prefab key |
| `UNKNOWN_PREFAB` | entity references unknown prefab |
