...
export function GenerationModal({
	...
	const [templateConfigs, setTemplateConfigs] = useState<TemplateConfig[]>(() =>
		Object.entries(prefabs).map(([id, prefab]) => ({
			id,
			name: id,
			entityPrompt: prefab.tags?.join(", ") ?? id,
			enabled: true,
		})),
	);
			...
			<View className="mb-6">
				...
				<View className="flex-row flex-wrap gap-2">
					...
					<Pressable
						...
						accessibilityState={{
							selected: !STYLE_PRESET_OPTIONS.some(
			...
			<View className="mb-6">
				...
				<ScrollView className="max-h-[250px]" nestedScrollEnabled>
					{templateConfigs.map((config) => (
						<View
							key={config.id}
							className="bg-theme-surface-elevated rounded-lg mb-2 overflow-hidden"
						>
							<Pressable
							...
							>
								...
								<Text className="flex-1 text-theme-text text-sm">
									{config.name}
								</Text>
								<Pressable
									className="p-1"
									...
									accessibilityRole="button"
									accessibilityLabel={
										expandedTemplate === config.id ? "Collapse" : "Expand"
									}
								>
							...
							{expandedTemplate === config.id && (
								<View className="p-3 pt-0 border-t border-theme-border">
									<Text className="text-theme-text-muted text-[11px] mb-1.5">
										Entity Prompt
									</Text>
									<TextInput
		...
		<View className="items-center py-12">
			...
			{lastGeneration?.compiledPrompt && (
				<View className="bg-theme-surface rounded-lg p-3 mt-4">
					...
					<Text className="text-theme-text-secondary text-sm leading-5">
						{lastGeneration.compiledPrompt}
					</Text>
				</View>
			)}
				...
				<View className="bg-theme-surface rounded-t-2xl max-h-[90%]">
					...
					<View className="flex-row justify-between items-start px-6 py-3 border-b border-theme-border">
						<View>
							...
							{packName && (
								<Text className="text-theme-text-muted text-xs mt-0.5">
									{packName}
								</Text>
							)}
						</View>
						<Pressable
							className="w-8 h-8 rounded-full bg-theme-surface-elevated items-center justify-center"
							onPress={onClose}
							accessibilityRole="button"
							accessibilityLabel="Close"
						>
					...
					<View className="p-6 pb-9 border-t border-theme-border">
						...
						{phase === "complete" && (
							<Pressable
								className="bg-theme-success py-3.5 rounded-lg items-center"
								onPress={onClose}
								accessibilityRole="button"
