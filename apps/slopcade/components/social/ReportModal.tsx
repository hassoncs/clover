...
const REASONS = [
  ...
  { key: "inappropriate" as const, label: "Inappropriate", icon: "warning-outline" as const },
  { key: "harassment" as const, label: "Harassment", icon: "alert-circle-outline" as const },
  { key: "other" as const, label: "Other", icon: "ellipsis-horizontal-outline" as const },
];
...
export function ReportModal({ visible, targetType, targetId, onClose }: ReportModalProps) {
  ...
  const reportMutation = trpcReact.moderation.report.useMutation({
    onSuccess: () => {
      setSubmitted(true);
    },
  });
                ...
                {REASONS.map((reason) => (
                  ...
                  >
                    <Ionicons
                      name={reason.icon}
                      size={20}
                      color={selectedReason === reason.key ? "#818CF8" : "#9CA3AF"}
                    />
                    <Text
                      className={`ml-3 text-sm font-medium ${
                        selectedReason === reason.key ? "text-indigo-400" : "text-gray-300"
                      }`}
                    >
                      {reason.label}
                    </Text>
