...
function compileShaderPlan(option: ShaderOption): CompiledPlan {
	...
	if (!compileResult.success || !compileResult.plan) {
		const details = compileResult.errors
			...
			.join(", ");
		throw new Error(
			`Failed to compile ${option.label} shader graph: ${details}`,
		);
	}

	const nodeShaders = new Map(
		option.spec.nodes.map((node) => [
			node.id,
			(node as ShaderNode).shader ?? "",
		]),
	);
	...
	useEffect(() => {
		...
		const initGame = async () => {
			try {
				...
				const defaultPlan = compileShaderPlan(
					SHADER_OPTIONS[selectedShaderRef.current],
				);
