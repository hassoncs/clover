...
export default function TextGridLab() {
	...
	const [silhouetteMode, setSilhouetteMode] =
		useState<(typeof SILHOUETTE_MODES)[number]>("fill");
	...
	const [stylePrompt, setStylePrompt] = useState(
		"puffy blue cartoon text, 3D rendered, playful, game title style",
	);
	const [previewMode, setPreviewMode] = useState<"text" | "grid" | "both">(
		"both",
	);
  ...
  <text x="${width / 2}" y="${height / 2}"
    ...
    ${text.replace(/\n/g, " ")}
		...
		for (let r = 0; r <= rows; r++) {
			...
			gridLines.push(
				`<line x1="0" y1="${y}" x2="${width}" y2="${y}" stroke="#444" stroke-width="1" stroke-dasharray="4,4"/>`,
			);
		}
		for (let c = 0; c <= cols; c++) {
			const x = c * cellWidth;
			gridLines.push(
				`<line x1="${x}" y1="0" x2="${x}" y2="${height}" stroke="#444" stroke-width="1" stroke-dasharray="4,4"/>`,
			);
		}
	...
	const generateStylized = useCallback(async () => {
		...
		try {
			const stylizeResponse = await fetch(
				`${env.apiUrl}/api/text-grid/stylize`,
				{
					method: "POST",
			...
			if (!stylizeResponse.ok) {
				const errData = await stylizeResponse
					.json()
					.catch(() => ({ error: { message: "Unknown error" } }));
				throw new Error(
					errData.error?.message || `HTTP ${stylizeResponse.status}`,
				);
			}
			...
			setError(
				err instanceof Error ? err.message : "Failed to generate stylized text",
			);
			setStylizedImageUrl(null);
		} finally {
			setGenerating(false);
		...
		<View style={[styles.container, isWide && styles.containerWide]}>
				...
				<View style={styles.row}>
					<View style={styles.buttonRow}>
						{ALIGN_OPTIONS.map((a) => (
							<TouchableOpacity
								key={a}
								style={[styles.button, align === a && styles.buttonActive]}
								onPress={() => setAlign(a)}
							>
					...
					<View style={styles.buttonRow}>
						{WRAP_MODES.map((w) => (
							<TouchableOpacity
								key={w}
								style={[styles.button, wrapMode === w && styles.buttonActive]}
			...
			<View style={[styles.preview, isWide && styles.previewWide]}>
				<ScrollView contentContainerStyle={styles.previewContent}>
					...
					{(previewMode === "grid" || previewMode === "both") &&
						gridSvgDataUrl && (
							<View style={styles.previewSection}>
