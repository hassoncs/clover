...
function AnchoredElement({
	...
	theme: Required<
		Pick<
			OverlayTheme,
			"textColor" | "fontSize" | "primaryColor" | "backgroundColor"
		>
	>;
	zIndex: number;
	onButtonPress?: (
	...
	if (element.visibleWhen && !evaluateCondition(element.visibleWhen, ctx))
		return null;
	...
	return (
		<View
			style={[
				...
				element.borderColor
					? { borderColor: element.borderColor, borderWidth: 1 }
					: undefined,
				applyOverlayStyle(element.style),
			]}
		>
...
function ButtonElement({
	...
	const isDisabled =
		element.disabled ||
		(element.disabledWhen
			? evaluateCondition(element.disabledWhen, ctx)
			: false);
	...
	return (
		...
		>
			{element.children.map((child) => {
				...
				if (child.visibleWhen && !evaluateCondition(child.visibleWhen, ctx))
					return null;
