import { type GodotBridgeBase, queryAsync as sharedQueryAsync } from "../query";
...
export function createGodotDebugBridge(): GodotDebugBridge | null {
	if (!isDebugEnabled()) {
		console.warn(
			"[GodotDebugBridge] Debug mode not enabled. Add ?debug=true to URL or run in dev mode.",
		);
		return null;
	}
	...
	const worldToScreen = (worldPos: Vec2): Vec2 => {
		const screenX =
			(worldPos.x - cameraPosition.x) * pixelsPerMeter * cameraZoom +
			viewportSize.width / 2;
		const screenY =
			(-worldPos.y + cameraPosition.y) * pixelsPerMeter * cameraZoom +
	...
	const screenToWorld = (screenPos: Vec2): Vec2 => {
		const worldX =
			(screenPos.x - viewportSize.width / 2) / (pixelsPerMeter * cameraZoom) +
			cameraPosition.x;
		const worldY =
			-(screenPos.y - viewportSize.height / 2) / (pixelsPerMeter * cameraZoom) +
			cameraPosition.y;
	...
	const getSnapshot = async (
		...
		if (supportsAsyncQuery) {
			const [worldResult, cameraResult, vpResult, snapshotResult] =
				await Promise.all([
					...
					queryAsync<{ timestamp?: number; entities: SceneNode[] }>(
						bridge,
						"getSceneSnapshot",
					).catch(() => ({ entities: [] })),
				]);
			...
			if (bridge.getSceneSnapshot) {
				...
				const sceneSnapshot = (bridge._lastResult ?? { entities: [] }) as {
					timestamp?: number;
					entities: SceneNode[];
				};
				...
				const transforms = (bridge._lastResult ?? {}) as Record<
					string,
					{ x: number; y: number; angle: number }
				>;
				bridge.getAllProperties();
				const properties = (bridge._lastResult ?? { entities: {} }) as {
		...
		if (options.filterPrefab) {
			filteredEntities = filteredEntities.filter(
				(e) => e.prefab === options.filterPrefab,
			);
		}
		...
		const entities: EntitySnapshot[] = limitedEntities.map((node) => {
			...
			if (detail === "high") {
				entity.physics = node.physics
					? {
							bodyType: node.physics.bodyType as
								| "dynamic"
								| "static"
								| "kinematic",
	...
	const captureScreenshot = async (
		...
		throw new Error(
			"[GodotDebugBridge] Screenshot capture not supported - captureScreenshot method not available on bridge",
		);
	};
	...
	const waitForCollision = async (
		...
		return waitForCondition(() => {
			const recent = state.collisionHistory.filter(
	...
	const debugBridge: GodotDebugBridge = {
		...
		query: async (
			...
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<QueryResult>(bridge, "query", [
				selector,
				options ?? {},
			]);
		},
		...
		setProps: async (
			entityId: string,
			values: Record<string, unknown>,
			options?: SetPropsOptions,
		): Promise<SetPropsResult> => {
			const bridge = getGodotBridge();
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<SetPropsResult>(bridge, "setProps", [
				entityId,
				values,
				options ?? {},
			]);
		...
		patchProps: async (
			ops: PatchOp[],
			options?: SetPropsOptions,
		): Promise<PatchPropsResult> => {
			const bridge = getGodotBridge();
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<PatchPropsResult>(bridge, "patchProps", [
				ops,
				options ?? {},
			]);
		},
		...
		destroy: async (
			...
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<DestroyResult>(bridge, "destroy", [
				entityId,
				options ?? {},
			]);
		},
		...
		clone: async (
			...
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<CloneResult>(bridge, "clone", [
				entityId,
				options ?? {},
			]);
		},
		...
		reparent: async (
			...
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<ReparentResult>(bridge, "reparent", [
				entityId,
				newParentId,
				options ?? {},
			]);
		...
		setSeed: async (
			seed: number,
			options?: { enableDeterministic?: boolean },
		): Promise<TimeControlResult> => {
			const bridge = getGodotBridge();
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<TimeControlResult>(bridge, "setSeed", [
				seed,
				options ?? {},
			]);
		},
		...
		unsubscribe: async (
			...
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<{ ok: boolean; error?: string }>(
				bridge,
				"unsubscribe",
				[subId],
			);
		...
		pollEvents: async (
			...
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<PollEventsResult>(bridge, "pollEvents", [
				options ?? {},
			]);
		},
		...
		getJoints: async (entityId?: string): Promise<GetJointsResult> => {
			...
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			if (entityId) {
				return queryAsync<GetJointsResult>(bridge, "getEntityJoints", [
					entityId,
				]);
			}
		...
		queryPoint: async (
			x: number,
			y: number,
			options?: { mask?: number; includeSensors?: boolean },
		): Promise<QueryPointResult> => {
			const bridge = getGodotBridge();
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<QueryPointResult>(bridge, "queryPoint", [
				x,
				y,
				options ?? {},
			]);
		...
		queryAABB: async (
			rect: AABB,
			options?: { mask?: number; includeSensors?: boolean },
		): Promise<QueryAABBResult> => {
			const bridge = getGodotBridge();
			if (!bridge)
				throw new Error("[GodotDebugBridge] Godot bridge not available");
			return queryAsync<QueryAABBResult>(bridge, "queryAABB", [
				rect,
				options ?? {},
			]);
		},
