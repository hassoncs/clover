...
export class TweenRuntimeSystem
	implements RuntimeSystem<TweenSystemConfig, TweenSystemState>
{
	readonly id = "tween";
		...
		this.tweenSystem =
			...
			new TweenSystem({
				setEntityPosition: (entityId, x, y) =>
					ctx.bridge.setPosition(entityId, x, y),
				setEntityRotation: (entityId, angle) =>
					ctx.bridge.setRotation(entityId, angle),
				setEntityScale: (entityId, scaleX, scaleY) =>
					ctx.bridge.setScale(entityId, scaleX, scaleY),
				setEntityOpacity: (entityId, opacity) =>
					ctx.bridge.setOpacity(entityId, opacity),
			});
	}
