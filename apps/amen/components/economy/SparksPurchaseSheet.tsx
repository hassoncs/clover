...
export function SparksPurchaseSheet({
	visible,
	onClose,
}: SparksPurchaseSheetProps) {
	const {
		data: products,
		isLoading,
		error,
	} = trpcReact.economy.getProducts.useQuery();
	...
	return (
		...
		>
			<View className="flex-1 justify-end bg-theme-background/80">
				<View className="bg-theme-surface rounded-t-3xl p-6 w-full border-t border-theme-border h-[80%]">
					...
					{isLoading ? (
						...
						<View className="flex-1 justify-center items-center">
							<Text className="text-theme-error mb-4">
