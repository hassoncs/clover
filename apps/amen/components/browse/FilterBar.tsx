...
interface FilterBarProps {
	...
	onSortByChange: (
		sort: "newest" | "popular" | "alphabetical" | "rating",
	) => void;
	showFilters: boolean;
...
const SORT_OPTIONS: {
	value: "newest" | "popular" | "alphabetical" | "rating";
	label: string;
}[] = [
	...
	return (
		<Pressable
			...
			className={`rounded-full mr-2 mb-2 ${compact ? "px-2.5 py-1" : "px-3 py-1.5"} ${
				selected ? "bg-theme-primary" : "bg-theme-surface-elevated"
			}`}
		...
		>
			<Text
				className={`${compact ? "text-xs" : "text-sm"} ${selected ? "text-theme-background font-medium" : "text-theme-text-secondary"}`}
			>
				{label}
			</Text>
		</Pressable>
		...
		<View>
			...
			<View className="flex-row items-center justify-between mb-4">
				...
				>
					...
					<Text className="text-theme-primary ml-2">
						{showFilters ? "▲" : "▼"}
					</Text>
				</Pressable>
				...
				{activeFilterCount > 0 && (
					...
					>
						<Text className="text-theme-text-secondary text-sm">Clear all</Text>
					</Pressable>
				)}
			</View>

			{showFilters && (
				<View className="mb-4 p-3 bg-theme-surface rounded-xl">
					<View className="mb-3">
						<Text className="text-theme-text-secondary text-xs mb-2 uppercase tracking-wide">
							Sort By
						</Text>
