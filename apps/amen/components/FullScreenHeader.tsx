...
export function FullScreenHeader({
	...
	return (
		<View
			className={`absolute top-0 left-0 right-0 z-10 ${showBackground ? "bg-theme-background/30" : ""}`}
		...
		>
			...
			>
				<Pressable
					className="py-1.5 px-3 bg-theme-background/40 rounded-lg backdrop-blur-sm"
					onPress={handleBack}
					accessibilityRole="button"
					accessibilityLabel="Back"
				>
					<Text className="text-theme-text font-medium text-sm">Back</Text>
				</Pressable>

				{centerContent ??
					(title ? (
						<Text
							className="text-theme-text font-bold text-base flex-1 text-center mx-2"
							numberOfLines={1}
						>
							{title}
						</Text>
