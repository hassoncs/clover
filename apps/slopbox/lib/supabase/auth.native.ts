...
const SLOPBOX_DOMAIN = "slopbox.tv";
const SLOPBOX_SCHEME = "slopbox";
...
const getWindowOrigin = () => {
	...
	return `https://${SLOPBOX_DOMAIN}`;
};

const REDIRECT_URL_WEB = getWindowOrigin() + "/auth/callback";
const REDIRECT_URL_NATIVE = `${SLOPBOX_SCHEME}://auth/callback`;
	...
	if (Platform.OS !== "web") {
		try {
			...
			const sessionKeys = keys.filter((k) =>
				k.startsWith("supabase_session_data_"),
			);
