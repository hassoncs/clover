...
function sanitizePatch(patch: UnknownRecord): UnknownRecord {
...
function updateNodeByPath(
	...
	if (isContainer(node) && Array.isArray(node.children)) {
		return updateNodeByPath(node.children, rest, patch);
	}
	...
	if (node.type === "ref") {
		...
		const previous = asRecord(descendants[descendantPath]) ?? {};
		...
		try {
			...
			if (type === "updateElement") {
				...
				if (elementId.includes("/")) {
					const updated = updateNodeByPath(
						nextDocument.children,
						elementId.split("/").filter((part) => part.length > 0),
						patch,
					);
					if (!updated) {
						errors.push(
							`op[${index}] updateElement target not found by path: ${elementId}`,
						);
						return;
					}
					appliedOps += 1;
