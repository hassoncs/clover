export function initialStreamState(threadId) {
    return {
        thread: {
            id: threadId,
            messages: [],
...
function appendTextContent(message, delta) {
...
function updateToolUseByToolCallId(messages, toolCallId, updater) {
    let changed = false;
...
function findToolUseBlock(messages, toolCallId) {
...
function findMessageIdForToolCall(messages, toolCallId) {
    for (const message of messages) {
        if (message.content.some((block) => block.type === 'tool-use' && block.toolCallId === toolCallId)) {
            return message.id;
        }
    }
    return null;
...
function resolveTargetMessageId(state, toolCallId) {
...
export function chatReducer(state, event) {
    switch (event.type) {
        ...
        case 'TEXT_MESSAGE_CONTENT': {
            const nextMessages = updateMessage(state.thread.messages, event.messageId, (message) => appendTextContent(message, event.delta));
        ...
        case 'TOOL_CALL_START': {
            const nextMessages = updateMessage(state.thread.messages, event.parentMessageId, (message) => ({
                ...message,
                content: [
                    ...message.content,
                    {
        ...
        case 'TOOL_CALL_ARGS': {
            ...
            const nextMessages = updateToolUseByToolCallId(state.thread.messages, event.toolCallId, (toolUse) => ({
                ...toolUse,
                args: parsedArgs,
            }));
            return {
        ...
        case 'TOOL_CALL_RESULT': {
            ...
            const nextToolMessages = updateToolUseByToolCallId(state.thread.messages, event.toolCallId, (previous) => ({
                ...previous,
                status: 'complete',
            }));
