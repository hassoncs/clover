...
import type {
	AngryBurnsLevelOverrides,
	SlopeggleLevelOverrides,
...
export class LevelLoader {
	...
	applyLevel(
		...
		const game = this.mergeLevelIntoGame(
			level,
			pack.gameConfig,
			baseGame,
			warn,
	...
	applyLevelById(
		...
		if (levelIndex === -1) {
			throw new Error(
				`Level "${levelId}" not found in pack "${pack.metadata.id}"`,
			);
		}
	...
	listLevels(
		pack: LevelPack,
	): Array<{ identity: string; levelId: string; title?: string }> {
	...
	private mergeLevelIntoGame(
		...
		if (gameConfig?.baseGameDefinition) {
			this.mergeConfig(
				game,
				gameConfig.baseGameDefinition as Partial<GameDefinition>,
				warn,
			);
		}
		...
		const slopeggleOverrides = level.overrides?.slopeggle as
			| SlopeggleLevelOverrides
			| undefined;
		...
		const angryBurnsOverrides = level.overrides?.angryBurns as
			| AngryBurnsLevelOverrides
			| undefined;
		...
		(game.metadata as unknown as Record<string, unknown>).generatorId =
			level.generatorId;
		(game.metadata as unknown as Record<string, unknown>).generatorVersion =
			level.generatorVersion;
		(game.metadata as unknown as Record<string, unknown>).levelSeed =
			level.seed;
	...
	private applySlopeggleOverrides(
		...
		// World dimensions
		if (
			overrides.worldWidth !== undefined ||
			overrides.worldHeight !== undefined
		) {
			...
			warn(
				"World bounds overridden - entities may need repositioning",
				"mergeWarnings",
			);
		}
		...
		if (overrides.hasBucket !== undefined) {
			const bucketEntity = game.entities.find(
				(e) => e.id === "bucket" || e.prefab === "bucket",
			);
			if (bucketEntity) {
				(bucketEntity as unknown as Record<string, unknown>).disabled =
					!overrides.hasBucket;
			}
		}
		...
		if (overrides.hasPortals !== undefined) {
			...
			if (portalA) {
				(portalA as unknown as Record<string, unknown>).disabled =
					!overrides.hasPortals;
			}
			if (portalB) {
				(portalB as unknown as Record<string, unknown>).disabled =
					!overrides.hasPortals;
			}
		}
	}
	...
	private applyAngryBurnsOverrides(
		...
		if (overrides.entities && overrides.entities.length > 0) {
			...
			if (replaceIds.size > 0) {
				warn(
					`Replaced ${replaceIds.size} entities from Angry Burns overrides`,
					"mergeWarnings",
				);
			}
		}
	...
	private validateGame(game: GameDefinition, warnings: string[]): void {
