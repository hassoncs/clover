...
function createMinimalBundle(overrides?: {
  ...
  // Minimal manifest
  const manifest = overrides?.manifest || { name: 'test-game', version: '1.0.0' };
  files.set('manifest.json', JSON.stringify(manifest));

  // Minimal templates
  const templates = overrides?.templates || [{ id: 'player', tags: ['player'] }];
...
describe('Virtual Bundle Integration', () => {
  ...
  it('errors on script without exports', () => {
    ...
    expect(result.errors[0].message).toContain('must contain at least one export');
  });
  ...
  it('produces valid GameDefinition', () => {
    const files = createMinimalBundle({
      ...
      templates: [
        { id: 'player', tags: ['player'], sprite: { type: 'circle', radius: 0.5 } },
        { id: 'enemy', tags: ['enemy'], sprite: { type: 'rect', width: 1, height: 1 } },
      ],
      rules: [
        {
          id: 'collision-rule',
    ...
    expect(gameDef.metadata.description).toBe('A test game for integration testing');
    ...
    expect(Object.keys(gameDef.templates)).toHaveLength(2);
  ...
  it('handles complex nested directory structure', () => {
    ...
    // Manifest
    files.set('manifest.json', JSON.stringify({ name: 'nested-test', version: '1.0.0' }));

    // Templates in subdirectories
    files.set('templates/player/player.json', JSON.stringify({ id: 'player', tags: ['player'] }));
    files.set('templates/enemies/basic.json', JSON.stringify({ id: 'basic-enemy', tags: ['enemy'] }));
    files.set('templates/enemies/boss.json', JSON.stringify({ id: 'boss-enemy', tags: ['enemy', 'boss'] }));

    // Rules in subdirectories
    files.set('rules/collision/player-enemy.json', JSON.stringify([
      { id: 'player-hit', trigger: { type: 'collision', entityATag: 'player', entityBTag: 'enemy' } },
    ]));
    files.set('rules/scoring/points.json', JSON.stringify([
    ...
    // Assets
    files.set('assets.json', JSON.stringify({
    ...
    expect(Object.keys(result.gameDefinition!.templates)).toHaveLength(3);
    expect(result.gameDefinition!.templates.player).toBeDefined();
    expect(result.gameDefinition!.templates['basic-enemy']).toBeDefined();
    expect(result.gameDefinition!.templates['boss-enemy']).toBeDefined();
  ...
  it('validates template references in entities', () => {
    ...
    files.set('manifest.json', JSON.stringify({ name: 'test', version: '1.0.0' }));
    files.set('templates/templates.json', JSON.stringify([{ id: 'player', tags: ['player'] }]));
    files.set('entities/entities.json', JSON.stringify([
      { id: 'player1', template: 'player', transform: { x: 0, y: 0, angle: 0 } },
      { id: 'enemy1', template: 'nonexistent', transform: { x: 5, y: 5, angle: 0 } },
    ]));
    files.set('rules/gameplay.json', JSON.stringify([]));
  ...
  it('resolves constant references in templates', () => {
    const files = new Map<string, string>();
    files.set('manifest.json', JSON.stringify({ name: 'test', version: '1.0.0' }));
    files.set('constants.json', JSON.stringify({
      PLAYER_SPEED: 5,
      GRAVITY: 9.8,
    }));
    files.set('templates/templates.json', JSON.stringify([
      {
        id: 'player',
        tags: ['player'],
    ...
    const playerTemplate = result.gameDefinition?.templates.player as any;
  ...
  it('errors on duplicate entity IDs', () => {
    ...
    files.set('entities/entities.json', JSON.stringify([
      { id: 'player1', template: 'player', transform: { x: 0, y: 0, angle: 0 } },
      { id: 'player1', template: 'player', transform: { x: 5, y: 5, angle: 0 } },
    ]));
    files.set('rules/gameplay.json', JSON.stringify([]));
  ...
  it('processes all file types correctly', () => {
    const files = createMinimalBundle({
      ...
      scripts: {
        ...
        update: 'exports.onUpdate = function(dt) { console.log("update", dt); };',
      },
      assets: {
        background: {
          type: 'image',
    ...
    expect(Object.keys(result.gameDefinition!.templates)).toHaveLength(2);
