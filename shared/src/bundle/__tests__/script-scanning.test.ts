...
describe('Script Scanning', () => {
  it('should process single script file and populate gameDefinition.script', () => {
    const files = new Map<string, string>([
      ...
      ['scripts/game.js', 'exports.onInit = function() { console.log("init"); };'],
    ]);
  ...
  it('should reject scripts with top-level return', () => {
    const files = new Map<string, string>([
      ['manifest.json', JSON.stringify({ name: 'test-game', version: '1.0.0' })],
      ['scripts/bad.js', 'exports.foo = 1;\nreturn 42;'],
    ]);
    ...
    expect(result.errors).toHaveLength(1);
    expect(result.errors[0].code).toBe('SCRIPT_SYNTAX_ERROR');
    expect(result.errors[0].message).toContain('top-level return');
  });

  it('should handle multiple exports in single file', () => {
    const files = new Map<string, string>([
      ['manifest.json', JSON.stringify({ name: 'test-game', version: '1.0.0' })],
      ['scripts/multi.js', 'exports.onInit = function() {};\nexports.onUpdate = function() {};'],
    ]);
