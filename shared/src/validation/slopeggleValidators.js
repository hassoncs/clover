 ...
 */
function fromCenterOrigin(cx, cy) {
 ...
 */
export function validateBounds(pegs) {
    const errors = [];
    const warnings = [];
    ...
    for (const peg of pegs) {
        ...
        if (peg.x - PEG_RADIUS < 0) {
            errors.push(`Peg "${peg.pegId}" at x=${peg.x.toFixed(3)} extends beyond left bound ` +
                `(x - radius = ${(peg.x - PEG_RADIUS).toFixed(3)} < 0)`);
        }
        // Check right bound
        if (peg.x + PEG_RADIUS > WORLD_WIDTH) {
            errors.push(`Peg "${peg.pegId}" at x=${peg.x.toFixed(3)} extends beyond right bound ` +
                `(x + radius = ${(peg.x + PEG_RADIUS).toFixed(3)} > ${WORLD_WIDTH})`);
        }
        // Check top bound
        if (peg.y - PEG_RADIUS < 0) {
            errors.push(`Peg "${peg.pegId}" at y=${peg.y.toFixed(3)} extends beyond top bound ` +
                `(y - radius = ${(peg.y - PEG_RADIUS).toFixed(3)} < 0)`);
        }
        // Check bottom bound
        if (peg.y + PEG_RADIUS > WORLD_HEIGHT) {
            errors.push(`Peg "${peg.pegId}" at y=${peg.y.toFixed(3)} extends beyond bottom bound ` +
 ...
 */
export function validateForbiddenZones(pegs) {
    const errors = [];
    const warnings = [];
    ...
    for (const peg of pegs) {
        ...
        if (peg.y < LAUNCHER_ZONE_Y_MAX) {
            ...
            errors.push(`Peg "${peg.pegId}" at y=${peg.y.toFixed(3)} is in launcher zone ` +
        ...
        if (peg.y > BUCKET_ZONE_Y_MIN) {
            ...
            errors.push(`Peg "${peg.pegId}" at y=${peg.y.toFixed(3)} is in bucket zone ` +
 ...
 */
export function validateSpacing(pegs) {
    const errors = [];
    const warnings = [];
    ...
    for (let i = 0; i < pegs.length; i++) {
        for (let j = i + 1; j < pegs.length; j++) {
            ...
            if (distSq < minSpacingSq) {
                ...
                errors.push(`Pegs "${pegA.pegId}" and "${pegB.pegId}" are too close: ` +
 ...
 */
export function validateOrangeCount(pegs, requestedCount) {
    const errors = [];
    const warnings = [];
    const actualOrangeCount = pegs.filter((p) => p.isOrange).length;
    if (requestedCount !== undefined) {
        if (actualOrangeCount !== requestedCount) {
            errors.push(`Orange peg count mismatch: requested ${requestedCount}, found ${actualOrangeCount}`);
        }
    }
    ...
    if (actualOrangeCount > 50) {
        warnings.push(`High orange peg count (${actualOrangeCount}) may make level too easy or chaotic`);
    }
    return {
        valid: errors.length === 0,
        errors,
 ...
 */
export function validateOrangeAccessibility(pegs) {
    const errors = [];
    const warnings = [];
    ...
    if (cornerRatio > 0.5) {
        warnings.push(`${corners.total}/${totalOranges} orange pegs (${(cornerRatio * 100).toFixed(0)}%) ` +
    ...
    if (corners.topLeft > 0) {
        warnings.push(`${corners.topLeft} orange(s) in top-left corner - may be unreachable with some angles`);
    }
    if (corners.topRight > 0) {
        warnings.push(`${corners.topRight} orange(s) in top-right corner - may be unreachable with some angles`);
    }
    if (corners.bottomLeft > 0) {
        warnings.push(`${corners.bottomLeft} orange(s) in bottom-left corner - bucket zone proximity may reduce accessibility`);
    }
    if (corners.bottomRight > 0) {
        warnings.push(`${corners.bottomRight} orange(s) in bottom-right corner - bucket zone proximity may reduce accessibility`);
    }
    // If all oranges are in corners, that's a serious issue
    if (corners.total === totalOranges && totalOranges > 3) {
        errors.push(`All ${totalOranges} orange pegs are in corner regions - layout likely inaccessible. ` +
 ...
 */
export function validateSlopeggleLevel(pegs, overrides) {
    const allErrors = [];
    const allWarnings = [];
    // Run all validators
    ...
    // Combine results
    allErrors.push(...boundsResult.errors, ...forbiddenResult.errors, ...spacingResult.errors, ...orangeCountResult.errors, ...accessibilityResult.errors);
    allWarnings.push(...boundsResult.warnings, ...forbiddenResult.warnings, ...spacingResult.warnings, ...orangeCountResult.warnings, ...accessibilityResult.warnings);
    return {
 ...
 */
export function createPegPositions(pegs, useCenterOrigin = false) {
...
//# sourceMappingURL=slopeggleValidators.js.map