const GODOT_INCOMPATIBLE_PATTERNS = [
    // Godot 3 → Godot 4 migration
    {
        ...
        suggestion: "Use 1.0 / TEXTURE_PIXEL_SIZE for texture dimensions in UV space",
    },
    {
...
function findLineNumber(source, _match, startIndex) {
    return source.substring(0, startIndex).split("\n").length;
}
function extractContext(source, matchIndex, _matchLength) {
...
export function lintShaderSource(source, path = "shader") {
    const issues = [];
    const errors = [];
    const warnings = [];
    ...
    for (const { pattern, code, severity, message, suggestion, } of GODOT_INCOMPATIBLE_PATTERNS) {
...
function isEffectGraphWithNodes(value) {
    return (typeof value === "object" &&
        value !== null &&
        ("nodes" in value || !Object.keys(value).length));
}
export function extractShaderSources(game) {
    const shaders = [];
        ...
        if (graph.nodes) {
            for (const node of graph.nodes) {
                if (node.params?.shaderSource) {
                    ...
                    else if (typeof shaderSource === "object" &&
                        shaderSource !== null &&
...
export function validateShaders(game, errors, warnings) {
...
export function autoFixShader(source) {
    let fixed = source;
    const changes = [];
    const FIXES = [
        // Godot 3 → 4
...
//# sourceMappingURL=shaderLinter.js.map