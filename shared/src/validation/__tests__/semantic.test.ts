...
describe("validateConstantRefs", () => {
	it("passes when all constant refs resolve", () => {
		...
		const game = makeGame({
			prefabs: {
				ball: {
					id: "ball",
					physics: { density: { const: "BALL_DENSITY" } },
				},
			} as unknown as GameDefinition["prefabs"],
		});

		validateConstantRefs(game, { BALL_DENSITY: 10 }, errors);
	...
	it("errors on dead constant reference", () => {
		...
		const game = makeGame({
			prefabs: {
				ball: {
					id: "ball",
					physics: { density: { const: "MISSING" } },
				},
			} as unknown as GameDefinition["prefabs"],
		});

		validateConstantRefs(game, { OTHER: 10 }, errors);

		expect(
			errors.some(
				(e) => e.code === "UNKNOWN_CONSTANT" && e.message.includes("MISSING"),
			),
		).toBe(true);
	});
	...
	it("errors on nested constant reference", () => {
		...
		expect(
			errors.some(
				(e) =>
					e.code === "UNKNOWN_CONSTANT" && e.message.includes("BALL_RADIUS"),
			),
		).toBe(true);
	});
});
