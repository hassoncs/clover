 ...
 */
function generateShaderMetadata(): string {
  const entries = Object.values(TEXT_SHADER_LIBRARY);
  return entries
    .map(
      (entry) => `- ${entry.id}: ${entry.aiHints.description}
 ...
 */
function generateMobileConstraints(tier: DeviceTier): string {
  const limits = {
    high: {
      maxEffects: 4,
      maxSamples: 16,
 ...
 */
export function generateTextEffectPrompt(
 ...
 */
export function generateAdjustmentPrompt(
  currentSpec: EffectGraphSpec,
 ...
 */
export function generatePresetSelectionPrompt(
  presetName: string,
  textContent: string,
  tier: DeviceTier = 'mid'
): string {
  ...
  const description = presets[presetName as keyof typeof presets] || presetName;
