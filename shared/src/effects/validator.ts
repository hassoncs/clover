...
function checkSelfLoops(
	...
	for (const conn of graph.connections) {
		if (conn.from.nodeId === conn.to.nodeId) {
			errors.push(
				err("E_SELF_LOOP", `Connection from "${conn.from.nodeId}" to itself`, [
					conn.from.nodeId,
				]),
			);
		}
	...
	for (const conn of graph.connections) {
		if (!nodeIds.has(conn.from.nodeId)) {
			errors.push(
				err(
					"E_MISSING_NODE_REF",
					`Connection references non-existent node "${conn.from.nodeId}"`,
					[conn.from.nodeId],
				),
			);
		}
		if (!nodeIds.has(conn.to.nodeId)) {
			errors.push(
				err(
					"E_MISSING_NODE_REF",
					`Connection references non-existent node "${conn.to.nodeId}"`,
					[conn.to.nodeId],
				),
			);
		}
	...
	for (const fb of graph.feedbackEdges) {
		if (!nodeIds.has(fb.from.nodeId)) {
			errors.push(
				err(
					"E_MISSING_NODE_REF",
					`Feedback edge references non-existent node "${fb.from.nodeId}"`,
					[fb.from.nodeId],
				),
			);
		}
		if (!nodeIds.has(fb.to.nodeId)) {
			errors.push(
				err(
					"E_MISSING_NODE_REF",
					`Feedback edge references non-existent node "${fb.to.nodeId}"`,
					[fb.to.nodeId],
				),
			);
		}
...
function checkDuplicateConnections(
	...
	for (const conn of graph.connections) {
		...
		if (seen.has(key)) {
			errors.push(
				err("E_DUPLICATE_CONNECTION", `Duplicate connection ${key}`, [
					conn.from.nodeId,
					conn.to.nodeId,
				]),
			);
...
function checkGeneratorHasInput(
	...
	for (const nodeId of nodesWithInput) {
		...
		if (node && node.family === "generator") {
			errors.push(
				err(
					"E_GENERATOR_HAS_INPUT",
					`Generator node "${nodeId}" must not have connected inputs`,
					[nodeId],
				),
			);
		}
...
function checkCycles(
	...
	for (const fb of graph.feedbackEdges) {
		feedbackSet.add(
			`${fb.from.nodeId}:${fb.from.output}->${fb.to.nodeId}:${fb.to.input}`,
		);
	}
...
function checkDisconnectedNodes(
	...
	const externalInputNames = new Set(
		graph.externalInputs?.map((ext) => ext.name) || [],
	);
	...
	for (const node of graph.nodes) {
		...
		const hasExternalInput = node.inputSlots.some((slot) =>
			externalInputNames.has(slot.name),
		);
		if (hasExternalInput) continue;

		errors.push(
			err("E_DISCONNECTED_NODE", `Node "${node.id}" has no connections`, [
				node.id,
			]),
		);
	}
