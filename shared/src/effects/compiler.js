...
function binarySearchInsert(sortedArr, value) {
    let lo = 0;
...
// ---------------------------------------------------------------------------
function buildConstraintEdges(constraints, nodeIds) {
    const edges = [];
...
// ---------------------------------------------------------------------------
function resolveShaderSource(node, shaderLookup) {
    const customGlsl = typeof node.params.shaderSource === "string"
...
// ---------------------------------------------------------------------------
function buildCompiledPass(node, resourceGraph, scope, shaderLookup) {
    const requires = [];
    const provides = [];
    const inputBindings = {};
    for (const binding of resourceGraph.bindings) {
        if (binding.passId !== node.id)
            continue;
    ...
    const shaderSource = resolveShaderSource(node, shaderLookup);
...
// ---------------------------------------------------------------------------
export function compileGraph(graph, options = {}) {
    ...
    for (const nodeId of topoResult.sorted) {
        ...
        passes.push(buildCompiledPass(node, resourceGraph, graph.scope, options.shaderLookup));
    }
    const resourceMap = {};
...
// ---------------------------------------------------------------------------
export function wrapShadersAsPlan(shaders, scope = "screen") {
    const passes = Object.entries(shaders).map(([id, glsl]) => ({
        id,
        shaderSource: { glsl },
        requires: [],
        provides: [],
