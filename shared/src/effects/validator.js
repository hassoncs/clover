...
function err(code, message, nodeIds, path) {
...
function checkEmptyGraph(graph, errors) {
...
function checkDuplicateNodeIds(graph, errors) {
    ...
    for (const node of graph.nodes) {
        if (seen.has(node.id)) {
            errors.push(err("E_DUPLICATE_NODE_ID", `Duplicate node id "${node.id}"`, [node.id]));
        }
        seen.add(node.id);
    }
}
function checkSelfLoops(graph, errors) {
    for (const conn of graph.connections) {
        if (conn.from.nodeId === conn.to.nodeId) {
            errors.push(err("E_SELF_LOOP", `Connection from "${conn.from.nodeId}" to itself`, [
                conn.from.nodeId,
            ]));
        }
    }
}
function checkMissingNodeRefs(graph, nodeIds, errors) {
    for (const conn of graph.connections) {
        if (!nodeIds.has(conn.from.nodeId)) {
            errors.push(err("E_MISSING_NODE_REF", `Connection references non-existent node "${conn.from.nodeId}"`, [conn.from.nodeId]));
        }
        if (!nodeIds.has(conn.to.nodeId)) {
            errors.push(err("E_MISSING_NODE_REF", `Connection references non-existent node "${conn.to.nodeId}"`, [conn.to.nodeId]));
        }
    }
    for (const fb of graph.feedbackEdges) {
        if (!nodeIds.has(fb.from.nodeId)) {
            errors.push(err("E_MISSING_NODE_REF", `Feedback edge references non-existent node "${fb.from.nodeId}"`, [fb.from.nodeId]));
        }
        if (!nodeIds.has(fb.to.nodeId)) {
            errors.push(err("E_MISSING_NODE_REF", `Feedback edge references non-existent node "${fb.to.nodeId}"`, [fb.to.nodeId]));
        }
    }
}
function checkDuplicateConnections(graph, errors) {
    const seen = new Set();
    for (const conn of graph.connections) {
        const key = `${conn.from.nodeId}:${conn.from.output}->${conn.to.nodeId}:${conn.to.input}`;
        if (seen.has(key)) {
            errors.push(err("E_DUPLICATE_CONNECTION", `Duplicate connection ${key}`, [
                conn.from.nodeId,
                conn.to.nodeId,
            ]));
        }
...
function checkGeneratorHasInput(graph, nodeMap, errors) {
    ...
    for (const nodeId of nodesWithInput) {
        ...
        if (node && node.family === "generator") {
            errors.push(err("E_GENERATOR_HAS_INPUT", `Generator node "${nodeId}" must not have connected inputs`, [nodeId]));
        }
    }
}
function checkCycles(graph, nodeIds, errors) {
    ...
    if (visited < nodeIds.size) {
        ...
        errors.push(err("E_GRAPH_CYCLE", `Graph contains a cycle involving nodes: ${cycleNodes.join(", ")}`, cycleNodes));
    }
}
function checkFeedbackLimit(graph, errors) {
    if (graph.feedbackEdges.length > 1) {
        errors.push(err("E_FEEDBACK_LIMIT", `Graph has ${graph.feedbackEdges.length} feedback edges (max 1 per subgraph in v1)`));
    }
}
function checkFeedbackInvalid(graph, nodeMap, errors) {
    for (const fb of graph.feedbackEdges) {
        const targetNode = nodeMap.get(fb.to.nodeId);
        if (targetNode && !targetNode.flags.stateful) {
            errors.push(err("E_FEEDBACK_INVALID", `Feedback edge targets non-stateful node "${fb.to.nodeId}"`, [fb.to.nodeId]));
        }
    }
}
function checkFormatMismatch(graph, nodeMap, errors) {
    for (const conn of graph.connections) {
        ...
        if (sourceFormat !== targetFormat) {
            errors.push(err("E_FORMAT_MISMATCH", `Format mismatch: "${conn.from.nodeId}" outputs ${sourceFormat} but "${conn.to.nodeId}" expects ${targetFormat}`, [conn.from.nodeId, conn.to.nodeId]));
        }
    }
}
function checkDisconnectedNodes(graph, nodeMap, errors) {
    ...
    for (const node of graph.nodes) {
        if (connectedNodes.has(node.id))
            continue;
        if (node.family === "generator")
            continue;
        const hasExternalInput = node.inputSlots.some((slot) => externalInputNames.has(slot.name));
        if (hasExternalInput)
            continue;
        errors.push(err("E_DISCONNECTED_NODE", `Node "${node.id}" has no connections`, [
            node.id,
        ]));
    }
}
function checkBudget(graph, options, errors) {
    if (!options.platformTier)
        return;
    const tier = BUDGET_TIER_PRESETS[options.platformTier];
    const passCount = graph.nodes.length;
    if (passCount > tier.maxPasses) {
        errors.push(err("E_BUDGET_EXCEEDED", `Graph has ${passCount} nodes but ${options.platformTier} allows max ${tier.maxPasses}`));
    }
}
