...
export function validateVariableOverrides(
	overrides: Record<string, GameVariableValue>,
	gameVariables: Record<string, GameVariable>,
	...
	for (const [key, overrideValue] of Object.entries(overrides)) {
		...
		if (typeof overrideValue !== "number") continue;

		if (!isVariableWithTuning(gameVar)) continue;
		if (!gameVar.tuning) continue;
...
export function applyVariableOverrides(
	gameVariables: Record<string, GameVariable>,
	overrideValues: Record<string, GameVariableValue>,
): Record<string, GameVariable> {
	const result: Record<string, GameVariable> = {};
