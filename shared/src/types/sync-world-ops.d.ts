...
import type { AnimateOptions, AnimateTarget, CloneOptions, RaycastOptions, ReparentOptions, SpawnOptions, WorldEntityData, WorldEntityQuery, WorldRaycastHit } from "./world-ops";
/**
...
export interface SyncWorldOps {
    ...
    spawnEntity3D(prefabId: string, position: Vec3, opts?: SpawnOptions): string | null;
    ...
    createVoxelBatch(voxels: Array<{
        x: number;
        y: number;
        z: number;
        color: string;
    }>): string;
    updateVoxelBatch(batchId: string, voxels: Array<{
        x: number;
        y: number;
        z: number;
        color: string;
    ...
    pixelBufferDraw(entityId: string, commands: Array<{
        type: string;
        [key: string]: unknown;
    }>): void;
    pixelBufferClear(entityId: string, color: string): void;
    animateEntity(entityId: string, target: AnimateTarget, opts?: AnimateOptions): void;
    ...
    win(): void;
    lose(): void;
    playSound(soundId: string, opts?: {
        volume?: number;
        pitch?: number;
    }): void;
    prepareVoice(voicePreset: string, text: string, opts?: VoicePrepareOptions): VoiceHandleId;
    isVoiceReady(handleId: VoiceHandleId): boolean;
    playVoice(handleId: VoiceHandleId, opts?: {
        volume?: number;
        pitch?: number;
    }): void;
    cameraShake(intensity: number, duration: number): void;
    cameraZoom(scale: number, duration?: number): void;
    applySpriteEffect(entityId: string, effect: SpriteEffectType, params?: SpriteEffectParams): string;
    updateSpriteEffectParam(entityId: string, effectId: string, paramName: string, value: unknown): void;
...
//# sourceMappingURL=sync-world-ops.d.ts.map