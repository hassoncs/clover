...
export type TagGroup = "world" | "prefabs" | "entities" | "scripts" | "effects" | "assets";
...
export type PackageErrorCode = "MANIFEST_INVALID" | "ARTIFACT_NOT_FOUND" | "ARTIFACT_HASH_MISMATCH" | "LOAD_FAILED" | "UNLOAD_FAILED" | "INSTANTIATE_FAILED" | "BRIDGE_ERROR" | "TIMEOUT";
...
export interface PackageRuntimeAPI {
    ...
    instantiatePrefab(prefabId: string, opts?: InstantiateOpts): Promise<EntityRef>;
    /** Instantiate from a Godot scene reference (opt-in lane for scene-backed prefabs). */
    instantiatePrefabFromScene(sceneRef: string, opts?: InstantiateOpts): Promise<EntityRef>;
...
export interface ArtifactResolver {
    resolve<T extends TagGroup>(manifest: BuildManifest, tag: T): Promise<TagPayload<T>>;
}
//# sourceMappingURL=PackageRuntime.d.ts.map