...
export function parseDesignDocument(data) {
    ...
    if (!result.success) {
        throw new DesignSchemaError(`Invalid design document schema: ${result.error.message}`);
    }
    return result.data;
}
export function isDesignDocument(data) {
    return DesignDocumentSchema.safeParse(data).success;
}
export function createEmptyDesignDocument(gameId, title) {
    const now = Date.now();
    return {
        version: "1.1",
        metadata: {
