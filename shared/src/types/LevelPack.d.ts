 ...
 */
export { type LevelDefinition, type LevelDifficultyParams, type DifficultyTier, type GeneratorInfo, type GameOverrides, type SlopeggleLevelOverrides, type PinballLevelOverrides, type VersionString, type Seed, isLevelDefinition, getLevelIdentity, validateLevelUniqueness, CURRENT_LEVEL_SCHEMA_VERSION, MIN_COMPATIBLE_SCHEMA_VERSION, } from './LevelDefinition';
/**
 ...
 */
export type PackCategory = 'tutorial' | 'standard' | 'challenge' | 'event' | 'community' | 'generated';
/**
 ...
 */
export declare function isLevelPack(value: unknown): value is LevelPack;
/**
 * Type guard to check if a value is a valid PackSummary.
 */
export declare function isPackSummary(value: unknown): value is PackSummary;
/**
 * Get a level from the pack by its levelId.
 */
export declare function getLevelById(pack: LevelPack, levelId: string): LevelDefinition | undefined;
/**
 * Get a level by its full identity: `${packId}:${levelId}`.
 */
export declare function getLevelByIdentity(pack: LevelPack, identity: string): LevelDefinition | undefined;
/**
 * Get levels ordered by their ordinal property.
 * Falls back to array order if ordinal is missing.
 */
export declare function getLevelsOrdered(pack: LevelPack): LevelDefinition[];
/**
 * Calculate pack statistics from level definitions.
 */
export declare function calculatePackStats(levels: LevelDefinition[]): PackStats;
/**
 * Generate pack summary from a full pack.
 */
export declare function generatePackSummary(pack: LevelPack, progress?: PackProgress): PackSummary;
/**
 * Validate pack-level identity uniqueness.
 */
export declare function validatePackLevelIdentities(pack: LevelPack): {
    valid: boolean;
    errors: string[];
};
/**
 ...
 */
export declare function isLevelUnlocked(pack: LevelPack, levelId: string, completedLevelIds: string[]): boolean;
/**
 * Current schema major version for LevelPack.
 */
export declare const CURRENT_PACK_SCHEMA_VERSION = 1;
/**
 * Minimum compatible schema version for parsing packs.
 */
export declare const MIN_COMPATIBLE_PACK_VERSION = 1;
