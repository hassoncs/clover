...
function createError(message, source, node, context) {
    return {
        message,
        source,
        position: node.start,
...
function inferType(node, ctx, source) {
    switch (node.type) {
        case 'NumberLiteral':
            return 'number';
        case 'BooleanLiteral':
        ...
        case 'Identifier': {
            ...
            ctx.errors.push(createError(`Unknown identifier: '${name}'`, source, node, ctx.path));
            return 'unknown';
        }
        case 'MemberAccess': {
            ...
            if (objType === 'vec2') {
                if (!VECTOR_PROPERTIES.has(prop)) {
                    ctx.errors.push(createError(`Unknown vector property: '${prop}'. Valid properties: x, y, r, g`, source, node, ctx.path));
                }
                return 'number';
            }
            ...
            if (node.object.type === 'Identifier' && node.object.name === 'variables') {
                if (!ctx.knownVariables.has(prop)) {
                    ctx.errors.push(createError(`Unknown variable: '${prop}'. Available variables: ${Array.from(ctx.knownVariables).join(', ') || 'none'}`, source, node, ctx.path));
                }
                return 'unknown';
            }
            if (node.object.type === 'MemberAccess' &&
                ...
                if (parentProp === 'transform') {
                    if (!TRANSFORM_PROPERTIES.has(prop)) {
                        ctx.warnings.push(`Unknown transform property: '${prop}'. Expected: x, y, angle`);
                    }
                    return 'number';
                }
        ...
        case 'FunctionCall': {
            if (!KNOWN_FUNCTIONS.has(node.name)) {
                ctx.errors.push(createError(`Unknown function: '${node.name}'`, source, node, ctx.path));
                return 'unknown';
            }
            ...
            switch (node.name) {
                ...
                case 'ceil':
                case 'round': {
                    const firstArgType = node.args.length > 0 ? inferType(node.args[0], ctx, source) : 'unknown';
...
export function validateExpression(source, options = {}) {
    const ctx = {
    ...
    try {
        ast = parse(source);
        returnType = inferType(ast, ctx, source);
    }
    catch (e) {
        const error = e;
...
export function validateAllExpressions(expressions, knownVariables = []) {
    const allErrors = [];
    const allWarnings = [];
