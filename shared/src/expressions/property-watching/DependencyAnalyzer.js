...
export class DependencyAnalyzer {
    ...
    handleMemberAccess(node, context) {
    ...
    determineWatchScope(node, context) {
        let current = node;
        while (current.type === "MemberAccess") {
            current = current.object;
        }
    ...
    computeStats() {
        ...
        const avgPropertiesPerEntity = entitiesAffected > 0 ? uniqueProperties.size / entitiesAffected : 0;
