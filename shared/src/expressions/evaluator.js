...
const BUILTIN_FUNCTIONS = {
    ...
    mix: (args) => {
        ...
        return BUILTIN_FUNCTIONS.lerp(args, {});
    },
    fract: (args) => {
...
function assertArgCount(name, args, expected) {
...
function resolveMemberAccess(obj, property) {
    if (isVec2(obj)) {
        switch (property) {
            case 'x':
            case 'r':
...
function evaluateVectorBinaryOp(op, left, right) {
    const a = asVec2(left);
    ...
    switch (op) {
        ...
        case '/':
            if (b.x === 0 || b.y === 0)
                throw new Error('Division by zero in vector');
            return { x: a.x / b.x, y: a.y / b.y };
        case '==':
            return a.x === b.x && a.y === b.y;
        case '!=':
...
function evaluateUnaryOp(op, operand) {
    switch (op) {
        case '-':
            if (isVec2(operand)) {
...
export function createSeededRandom(initialSeed = 12345) {
