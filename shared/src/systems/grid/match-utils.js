 ...
 */
export function getHexNeighbors(row, col, isOddRow) {
    if (isOddRow) {
        return [
            { row: row - 1, col },
            { row: row - 1, col: col + 1 },
 ...
 */
export function floodFill(grid, startRow, startCol, matchFn, getNeighborsFn = getNeighbors) {
    ...
    while (queue.length > 0) {
        ...
        if (cell === undefined || !matchFn(cell, startCell))
            continue;
 ...
 */
export function findConnectedGroups(grid, minSize, matchFn, getNeighborsFn = getNeighbors) {
    const rows = grid.length;
    if (rows === 0)
        return [];
    const visited = new Set();
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < rowCells.length; col++) {
            ...
            if (cell === undefined)
                continue;
            const group = floodFill(grid, row, col, (c, start) => matchFn(c, start), getNeighborsFn);
 ...
 */
export function findLineMatches(grid, minLength, matchFn, directions = DEFAULT_LINE_DIRECTIONS) {
    const rows = grid.length;
    if (rows === 0)
        return [];
    const matches = [];
    ...
    for (let row = 0; row < rows; row++) {
        ...
        for (let col = 0; col < rowCells.length; col++) {
            const cell = rowCells[col];
            if (cell === undefined || !matchFn(cell))
                continue;
            for (const dir of directions) {
                const line = checkLineFromPosition(grid, row, col, dir.dRow, dir.dCol, minLength, matchFn);
                if (line) {
...
function checkLineFromPosition(grid, startRow, startCol, dRow, dCol, minLength, matchFn) {
    const rows = grid.length;
