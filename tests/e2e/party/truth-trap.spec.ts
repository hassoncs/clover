...
test.describe("Truth Trap E2E", () => {
	test("full truth trap game flow: create → join → start → lies → vote → winner", async () => {
		...
		for (let round = 1; round <= ROUND_COUNT; round++) {
			const writingState = await waitForMessage(
				host.messages,
				(msg) =>
			...
			expect(writingState.state?.sharedData?.prompt).toBeTruthy();
			expect(writingState.state?.sharedData?.multiplier).toBeTruthy();

			for (const p of players) {
				const reqStart = p.conn.messages.length;
				const liesReq = await waitForMessage(
					p.conn.messages,
					(msg) => msg.type === "input_request" && msg.requestId === "lies",
					30_000,
					reqStart,
				);
				expect(liesReq.request?.type).toBe("text");
				sendInputResponse(
					p.conn.ws,
					"lies",
					`${p.name} lie for round ${round}`,
					p.name,
				);
			}
			...
			expect(votingState.state?.sharedData?.answers).toBeTruthy();
			...
			for (const p of players) {
				...
				sendInputResponse(p.conn.ws, "votes", 0, p.name);
			}
